...
`npm run dev` でプレビューを開き:

1. 計測を数回行い、リードを1つ登録して、記録がある状態を作る
2. 書き出しのボタンを押す → `ficus-backup-YYYY-MM-DD.json` がダウンロードされる
3. **ファイルを開き、`format` / `version` / `counts` / `kv` / `sessions` が揃っていることを目視**
4. `sessions[0].frames` が空でないこと(生データが入っていること)

- [ ] **Step 2: 読み戻しを確認**

1. ブラウザの開発者ツールで IndexedDB を**削除**する(全損を再現)
2. アプリを再読み込み → 記録が空になっていることを確認
3. 読み戻しで Step 1 のファイルを選ぶ → 確認が出る → OK
4. **計測の履歴・リード・目安・設定がすべて戻っていること**を画面で確認
5. My Data のグラフが以前と同じ形を描くこと

- [ ] **Step 3: 壊れたファイルを弾くことを確認**

1. 適当な `.json`(`{}` だけのファイル等)を選ぶ → 日本語のエラーが出て、**既存データが消えない**
2. `version` を 99 に書き換えたファイルを選ぶ → 「新しいバージョン」の旨のエラー

- [ ] **Step 4: 保存の永続化を確認**

開発者コンソールで `await navigator.storage.persisted()` を実行し、`true` が返ること。
`false` の場合、画面に注意が出ていることを確認(ブラウザによっては申請が通らない。それ自体は異常ではない)

- [ ] **Step 5: 既存機能が壊れていないことを確認**

計測タブで録音 → 保存 → リードタブ → My Data を一巡し、**この計画の前と同じ挙動**であること

- [ ] **Step 6: Commit**

```bash
git commit --allow-empty -m "記録の保全: 実機確認を完了"
```

...
const code = [
  ...
  // 【N-11 2026/08/17 本人指示】My Data のグラフだけフラジオを含めないための音数
  extractConst("SAX_ALTISSIMO_SEMITONES"),
  extractFunction("noteAxisCount"),
  ...
  // F-107: ページ間の溝(トークンのまま)と、静止時・ドラッグ中で共有する transform の式
  extractConst("SWIPE_PAGER_GUTTER"),
  extractFunction("swipePagerTrackTransform"),
...
const api = new Function(`${code}
  return { freqToNote, centsBetween, writtenNoteLabel, parseNoteLabel, writtenMidiToSoundingFreq,
           buildFingeringTable, noteAxisCount, SAX_ALTISSIMO_SEMITONES,
           findClosestFingering, fftRadix2, detectPitchMPM, computeTimbreMetrics,
...
console.log("=== 検証19: 環の配色(OKLCH)・帯のグラデーション・到達の演出 ===");
{
  ...
  // ------------------------------------------------------------------
  {
    ...
    // 実寸(CSS px)。光は環の外へ出るので SVG の overflow を切ってはいけない。
    {
      ...
      // これなら片方だけ消しても落ちる。
      // 【F-71 2026/08/26 で穴を塞いだ】当て先が PitchRing **全体**だったので、
      // svg から両方消し、同じ綴りを同じ順序で **PitchRing 内の別要素**へ移すと通っていた
      // (作為的だが塞げる穴。N5)。当て先を **<svg の開きタグ**に絞る。
      // 空回りしないよう、切り出した開きタグが実在する長さであることも同時に見る。
      const svgOpen = (() => {
        const i = ringCode.indexOf("<svg");
        const j = ringCode.indexOf(">", i);
        return i === -1 || j === -1 ? "" : ringCode.slice(i, j + 1);
      })();
      check("光を出す SVG は overflow:visible かつ当たり判定を持たない(<svg の開きタグの中で隣接)",
        svgOpen.length > 40 && /overflow: "visible", pointerEvents: "none"/.test(svgOpen),
        svgOpen.replace(/\s+/g, " ").slice(0, 220));
...
// 作法が丸ごと無効になる。ここはそれを構造で見張るための節。
{
  ...
  // 【D-3a 2026/08/26 で下限を 2→1 に下げた】TappableMetricCard の非bare枝(.tile)は N-9 で
  // 呼び出し0件の死枝になっており、部品ごと削除したので **.tile は MetricCard の1つだけ**に
  // なった(実測 1)。0 にはならない = この検査はまだ何かを見ている。
  // **下げたぶんは、下で「セッション詳細・PhraseTimeline・
  // データタブに .card / .tile / .tile-row が1つも無い」ことを名指しで固定して埋める。**
  check(".card が実際に使われている", cardTags.length >= 1, `${cardTags.length}箇所`);
  check(".tile が実際に使われている", tileTags.length >= 1, `${tileTags.length}箇所`);
  // 【D-3a】下限を下げたので、**残った1つが誰のものか**を名指しで固定する
  // (下限だけを下げると「.tile が全部消えた」も通ってしまう)。
  check("D-3a: 残った .tile は計測タブの詳細(MetricCard)のもの",
    /className="tile"/.test(srcOfFn(src, "MetricCard")), `${tileTags.length}箇所`);
  ...
  // **「箱が無い」ことの十分条件ではない**(インラインで塗れば別・親が巻けば別)ので名前もそう名乗る。
  {
    ...
    // 【D-9z2 2026/08/26】My Data のカードには上余白を詰める .card-tight-top が足された。
    // 「.card を名乗っている」という主張は変わらないので、**後ろに続くクラスを許す**
    // (綴りを1つずつ足すと、次に例外クラスが増えるたびにここが落ちる)。
    check("D-1/D-2: データ系のカードは実際に .card を名乗っている(作法の外で箱を作っていない)",
      owners.every((fn) => /className="card(?: [a-z-]+)*"/.test(srcOfFn(src, fn))),
      owners.filter((fn) => !/className="card(?: [a-z-]+)*"/.test(srcOfFn(src, fn))).join(" / ") || "全部 .card");
    {
  ...
  // **「面が無い」ことの十分条件ではない**(親が .card を巻けばここは通る)ので、
  // 名前もその範囲しか名乗らない。
  ...
  // 常時表示の3行は「グラフは常に1枚」の指示で廃止され、関数ごと消えた(黙って消していない)。
  {
    ...
    check("N-6: My Data 子タブの2関数に .card が1枚も無い(§6.0 の囲いの序列)",
  ...
  // 対象外(従来どおり全音域)。渡し忘れ・渡しすぎのどちらも壊れたら検出できるようにする。
  {
    ...
    // **呼び手はゼロになった**。
    // 【D-3a 2026/08/26】呼び手ゼロの部品を残さない方針で**定義ごと削除した**ので、
    // 主張を「呼び手が0件」から「定義も呼び出しも1つも無い」へ強める。
    // 綴りは raw の src で見る(コメントに書いた墓碑は `function` / `<` を伴わない)。
    // **空回りしていないこと**を、同じ形で現存する部品(MetricTabCard)が引っかかることで示す。
    const tappableDef = /function\s+TappableMetricCard/.test(src);
    const tappableUse = /<TappableMetricCard/.test(src);
    check("D-3a: TappableMetricCard は定義も呼び出しも残っていない(読み手ゼロの部品を残さない)",
      !tappableDef && !tappableUse && /function\s+MetricTabCard/.test(src),
      `定義 ${tappableDef} / 呼び出し ${tappableUse} / 対照(MetricTabCard の定義) ${/function\s+MetricTabCard/.test(src)}`);
  ...
  // NoteAxisLineChart 自体が noteFocus を実装として持っている
  // (呼び出し側だけ書いて実装が無い、という状態を防ぐ)。
  {
    ...
    // (【D-3a 2026/08/26】「TappableMetricCard が noteFocus をそのまま渡す」検査はここにあったが、
    //  部品ごと削除したので落とした。渡し手が居ないことは上の検査が固定している。)
  }
  ...
  // 1本のはずの線が2本に戻り、指示に反する)。
  {
    ...
    // 期待値をここに写すのではなく、**正典の該当行から読む**(写すと正典が変わっても落ちない)。
    {
      ...
      // データタブに**実際に出る**音色3ラベルが、正典データタブの節の .l と一致すること。
      // キーの並びを辿って解決するので、配列側の定義が変わると落ちる。
      // 【N-10 2026/08/17 本人指示による書き換え】並びの出どころが MY_DATA_ROW_METRICS
      // (廃止した常時表示3行のキー列)から MY_DATA_CARD_METRICS(数字カード4枚のキー列)へ移った。
      // カードはピッチ(平均差分)を先頭に持つので、音色3指標だけを**並び順のまま**取り出す
      // (件数で切らずにピッチのキーを除くので、カードが増減しても正しく追従する)。
      {
        const cardKeys = new Function(`${extractConst("MY_DATA_CARD_METRICS")} return MY_DATA_CARD_METRICS;`)();
        const rowKeys = cardKeys.filter((k) => k !== "pitchCentsSigned");
        check("N-10: 数字カードのキー列から音色3指標を並び順のまま取り出せている(空回りしていない)",
          rowKeys.length === 3 && cardKeys[0] === "pitchCentsSigned", cardKeys.join("/"));
        const myData = extractConst("MY_DATA_METRICS");
  ...
  // 動く側(コメント除去後)のソースに1つも現れないことを固定する(経緯はコメントに書き残してよい)。
  {
    ...
    // 【N-10 2026/08/17 本人指示で1語だけ外した】「ばらつき」は正典 案K が**グラフの帯の凡例**
    // として使う語になったので、禁止語から外す。数字の表記が「標準偏差」1つであることは
    // 21.4 の「標準偏差は1箇所だけ」と「ばらつきは帯の凡例の1箇所だけ」が引き続き固定する。
    for (const old of ["平均ピッチ誤差", "ピッチの安定度", "ピッチ誤差(絶対値)", "平均ピッチ偏差", "ピッチ誤差"]) {
  ...
  // "-" が構造上出得なかった(BACKLOG F-37)。良否の色分けは今までどおり絶対値で評価する。
  {
    ...
    // 【N-10 2026/08/17 本人指示で書き換え】ヒーローは廃止され、ピッチは**数字カードの1枚**に
    // なった。符号付き(pitchCentsSigned)を使うという芯は変わらないので、錨だけ移す
    // (母集団の綴りは dayMetrics / overall のまま)。
    // 【N-11 2026/08/17 本人指示で書き換え】数字カード4枚も廃止され、数字は**グラフカードの中の
    // 1つだけ**になった(本人「数字が常に出ている必要はない」)。**直近日の数字そのものが
    ...
    // 【F-56 → N-10】良否の3段評価(Great/Good/Keep Trying)は本人指示で**廃止**された
    // (正典 案K は「事実だけを出す(解釈しない)」)。絶対値で評価する芯ごと無くなったので、
    // 主張を「良否の判定がもう無い」へ反転させる(21.8 が語と色まで固定する)。
    check("N-10: 良否の判定(dayErr / periodErr)が動く側に残っていない",
      !/const dayErr\b/.test(codeOf(myDataSection)) && !/const periodErr\b/.test(codeOf(myDataSection)));
...
console.log("=== 検証20: F-51 振り子 / F-52 音声時計の停止 / F-53 画面ぶんの高さ ===");
{
  ...
  //               ここが本体。1つでも背面に落ちると「押しても何も起きない」が生まれる(§6.1.5)。
  {
    ...
    // 符号だけ変えて書く(別管理にすると安全域のある端末でずれる)。
    {
      ...
      // 【N-11 2026/08/17】左右の値は式ではなく**トークン1つ**(--page-pad-left /
      // --page-pad-right)になった。トークンのときの反転は `calc(-1 * var(...))`
      // (`calc(-var(...))` は CSS として不正)。**計算値は従来と 1px も変わらない**。
      const neg = (v) => {
        const s = String(v).trim();
        if (/^var\(--[a-z0-9-]+\)$/.test(s)) return `calc(-1 * ${s})`;
        const m = /^calc\((.*)\)$/.exec(s);
        const body = m ? m[1] : s;
...
console.log("=== 検証21: F-66 平均差分(ピッチ)に標準偏差を併記 ===");
{
  ...
  // F-46 で SESSION_METRICS を廃止して1配列に統合したのと同じ考え方。
  {
    ...
    // 【N-10 2026/08/17 本人指示で書き換え】正典 案K がグラフの帯を「ばらつき」と呼ぶので、
    // **凡例の語としての「ばらつき」だけ**を許す(検証27 の 27.6 が正典と突き合わせている)。
  ...
  // (ハーネスはJSXを評価しないので、ここは実ソースの綴りで縛る。この節の限界として明記する)
  {
    ...
    // 平均差分は**数字カードの1枚**として正しく画面に出るようになったので、主張を
    // 「4指標が1枚ずつ・重複なし・既定選択は平均差分」へ反転させる(黙って消していない)。
    {
      const cardKeys = new Function(`${extractConst("MY_DATA_CARD_METRICS")} return MY_DATA_CARD_METRICS;`)();
      check("21.5b N-10: 数字カードは 平均差分 / HNR / 重心 / 音量 の4枚(本人指示の並び)",
        JSON.stringify(cardKeys) === JSON.stringify(["pitchCentsSigned", "hnrDb", "spectralCentroidHz", "volumeDb"]),
        cardKeys.join(","));
      check("21.5b N-10: 同じ指標を2枚出さない(キーが重複していない)",
        new Set(cardKeys).size === cardKeys.length, cardKeys.join(","));
    }
    const myDataSection = componentSourceOf("MyDataSection");
    // 【N-10 2026/08/17 本人指示による書き換え】旧 21.5 は「ヒーローの標準偏差(±3.4¢)は主役の
    // 数字と同じ母集団から / 語を持たない形で出す」を見ていた。紺のヒーローは案K の採用で廃止され、
    // **ばらつきは数字ではなく音ごとの帯**として出るようになったので、その配線に置き換える。
    // pitchSpreadSub(語つき/語なしの導出)そのものはリード詳細・セッション詳細が使い続けており、
    ...
    // 【N-10 2026/08/17 本人指示で書き換え】ヒーローの副次行(±3.4¢ · 平均 −0.8¢)は
    // 紺のカードごと廃止された。散らばりは**音ごとの帯**として出る(27.5)。
    ...
    // 【D-3a 2026/08/26】副次(sub)の受け口を持っていた TappableMetricCard は削除した。
    // 副次の行がいま生きているのは **PhraseTimeline のドリルダウン**(正典 .numrow .b)。
    // 「値が無い列でも場所を確保する」(§6.1.5)は17節が固定しているので、ここでは
    // **副次が数値の列の一部として実在する**ことだけを見る。
    const numrowCode = componentSourceOf("PhraseTimeline");
    check("21.5 D-3a: 副次(sub)の行は数値の列の中に残っている(正典 .numrow .b)",
      /BARE_ROW_STYLES\.numrow\.sub/.test(numrowCode) && /\bsub:/.test(numrowCode));
  }
...
console.log("=== 検証22: F-54 音名を実音へ / F-56 3段評価 / F-57〜F-60 ===");
{
  ...
  // --- 21.8 【N-10 2026/08/17 本人指示で反転】評価ピル(Great/Good/Keep Trying)の廃止 -----
  // 旧 21.8 は F-56 の3段評価(0からの距離で Great / Good / Keep Trying)を、実ソースの判定
  // ブロックをそのまま評価して固定していた。**N-10 で紺のヒーローカードごと廃止**された
  // (本人指示: 正典 案K は「事実だけを出す(解釈しない)」)ので、主張を
  // 「その解釈が画面から消えている」へ反転させる(黙って消していない)。
  {
    const code = codeOf(src);
    const WORDS = ["Great", "Good", "Keep Trying"];
    const alive = WORDS.filter((w) => code.includes(`"${w}"`));
    check("21.8 N-10: 評価の語(Great / Good / Keep Trying)が動く側に1つも無い",
      alive.length === 0, alive.join(",") || "0件");
    // 旧3段の色(ミント/緑/オレンジ)も一緒に消えていること(語だけ変えて残す形を許さない)
    const COLORS = ["#6EE7B7", "#4ADE80", "#FBBF24"];
    const aliveColors = COLORS.filter((c) => code.includes(c));
    check("21.8 N-10: 旧3段の評価色(#6EE7B7 / #4ADE80 / #FBBF24)も残っていない",
      aliveColors.length === 0, aliveColors.join(",") || "0件");
    check("21.8 N-10: 判定の綴り(heroStatus / heroColor / dayErr)も残っていない",
      !/heroStatus|heroColor|dayErr/.test(code));
    // 旧4段の文言(N-6 以前)も引き続き残っていないこと(21.8 の旧検査から引き継ぐ)
    check("21.8 旧4段の文言(ほぼ完璧/平均より改善/平均より悪化/平均並み)が動く側に残っていない",
      ["ほぼ完璧", "平均より改善", "平均より悪化", "平均並み"].every((t) => !code.includes(t)));
    // 紺の面そのものが1枚も無いこと(案K の前提)。My Data の2関数に限って名乗る。
    const noSurface = ["MyDataSection", "MyDataPage"]
      .filter((n) => /background: "var\(--c-accent\)"/.test(componentSourceOf(n)));
    check("21.8 N-10: My Data の2関数に紺の面(--c-accent の background)が1枚も無い",
      noSurface.length === 0, noSurface.join(",") || "0件");
  }
...
let METRO_SIGS_ALL = [];
{
  ...
  // --- 24.8 アップロードはデータタブで完結する --------------------------------
  {
    ...
    // ボタン**(正典 = mydata-v2-proposals.html の案P の .fab)へ移した。見出しの行から
    // 取り込みは無くなり、残るのは「選択」だけ。**機能=隠しファイル入力の click は不変**なので、
    // そこは主張を変えずに残す(浮かせるボタン側の作法は検証31 が持つ)。
    check("N-11: データタブの取り込みは浮かせるボタン(FloatingAction)から開く",
      /<FloatingAction[\s\S]{0,400}?onClick=\{\(\) => uploadInputRef\.current\?\.click\(\)\}/.test(dv));
    check("データタブの取り込みは隠しファイル入力を開く(機能は不変)",
      /onClick=\{\(\) => uploadInputRef\.current\?\.click\(\)\}/.test(dv));
    check("N-11: 取り込みの入口は1つだけ(見出しの行から浮かせるボタンへ移り、二重に置いていない)",
      (dv.match(/uploadInputRef\.current\?\.click\(\)/g) || []).length === 1,
      `${(dv.match(/uploadInputRef\.current\?\.click\(\)/g) || []).length}箇所`);
...
console.log("\n========== 検証25: N-5 リードタブ(正典 north-star-measure.html との突き合わせ) ==========");
{
  ...
  // --- 25.4 【F-111 2026/08/17 本人指示で反転】一覧末尾の「＋ 追加」の廃止 ---------
  // 旧 25.4 は正典 north-star-measure.html の .addrow / .addbtn(中央寄せ / padding-top 20 /
  // 14px の --accent)と実装を突き合わせていた。本人の課題「リードが増えるほど『＋追加』が
  // 下に流れ、毎回スクロールが必要」により、入口は**右下に浮かせるボタン**(正典 =
  // design/detail-and-reed-proposals.html の案D)へ移った。
  // したがって主張を「.addrow / .addbtn の形はもう無い」「入口は浮かせるボタン1つ」へ
  // 反転させる(黙って消していない)。浮かせるボタン自体の作法は検証31 が持つ。
  {
    // 正典側(旧)の規則は今も読める = この検査が空回りしていないことの裏取り
    check("F-111: 旧正典の .addrow / .addbtn の規則自体は読める(空回りしていない)",
    ...
    check("F-111: 浮かせるボタンに reeds.length の条件が付いていない(0枚でも出る)",
      /\{pageActive && \(\s*\r?\n\s*<FloatingAction/.test(codeOf(srcOfFn(src, "ReedRegisterView"))),
      (codeOf(srcOfFn(src, "ReedRegisterView")).match(/\{[^\n]{0,40}&& \(\s*$/gm) || []).slice(-2).join(" / "));
  ...
  // 純関数なので**全マスを総当たり**する。座標 → 落ちる位置が格子どおりであること。
  {
    ...
    // 「終わり方の全パターン × track に残る値」を総当たりで突き合わせる。
    {
      ...
      // 【審査で塞いだ穴 2026/08/17】しきい値の元にする幅は**1ページの幅**であって
      // viewport の clientWidth ではない。N-11 の bleed(グラフカードを画面いっぱいにするため
      // viewport へ左右 padding を入れる)で両者が食い違い、**データタブだけ確定のしきい値が
      // 69.40→75.00px(+8.07%)に重くなっていた**(審査役の実測)。ドラッグして戻る距離は
      // ページ幅+溝のままなので、判定だけ別の寸法になっていた。
      // 純関数 swipePagerPageWidth で padding を引くことと、それを endGesture が使うことを固定。
      {
        const pw = new Function(`${extractFunction("swipePagerPageWidth")}
          return swipePagerPageWidth;`)();
        const fake = (cw, l, r) => ({ clientWidth: cw });
        // getComputedStyle が無い環境(ハーネス)では padding 0 扱い = clientWidth をそのまま返す
        check("F-107: ページ幅は viewport が無ければ 0(落ちない)", pw(null) === 0 && pw(undefined) === 0);
        check("F-107: ページ幅は padding を引いた content box(bleed でしきい値が動かない)",
          pw(fake(375)) === 375 && /clientWidth \|\| 0\) - padL - padR/.test(extractFunction("swipePagerPageWidth")),
          String(pw(fake(375))));
        check("F-107: 終端はページ幅を渡す(clientWidth を直接渡さない)",
          /swipePagerEndKind\(s\?\.horizontal, interrupted, s\?\.dx \?\? 0,\s*\r?\n\s*swipePagerPageWidth\(viewportRef\.current\), i, count\)/
            .test(srcOfFn(src, "SwipePager")));
      }
      ...
      // 【F-107 2026/08/17 本人指示で書き換え】ページ間に溝が入ったので、
      // 期待値は式の写しではなく**画面上の位置(px)**で確かめる。
      // 実装が返す calc(...) を「トラック幅 W / 溝 G」で数値に解き、
      // 「index 番のページの左端が viewport の左端(0)に来ること」を見る。
      // 溝の実値 G は **index.css のトークンから読む**(実装にも検査にも px を写さない)。
      const cssTok = readFileSync(join(__dirname, "..", "src", "index.css"), "utf8");
      const G = Number((/--sp-4:\s*(\d+)px/.exec(cssTok) || [])[1]);
      check("F-107: 溝の値を index.css のトークン(--sp-4)から読めている(空回りしていない)",
        Number.isFinite(G) && G > 0 && api.SWIPE_PAGER_GUTTER === "var(--sp-4)",
        `${api.SWIPE_PAGER_GUTTER} = ${G}px`);
      // calc の文字列 → px。読めない形は NaN にして必ず落とす。
      const evalTx = (s) => {
        const m = /^translateX\(calc\((-?[\d.]+)% - (\d+) \* var\(--sp-4\) \+ (-?[\d.]+)px\)\)$/.exec(String(s));
        if (!m) return NaN;
        return (Number(m[1]) / 100) * W - Number(m[2]) * G + Number(m[3]);
      };
      {
        const bad = [];
        let n = 0;
        for (const flag of [false, true]) {
          for (const i of [0, 1]) {
            for (const dx of [-300, -TH - 1, -TH, -TH + 1, -80, -8, -0.5, 0, 0.5, 8, 80, TH - 1, TH, TH + 1, 300]) {
              ...
              // transform: advance は React が書く(null)。それ以外は
              // **index 番のページの左端が 0 に来る位置**(= 溝込みで -i*(W+G))。
              if (k === "advance") {
                if (style.transform !== null) bad.push(`flag=${flag} i=${i} dx=${dx} → advance なのに ${style.transform}`);
                continue;
              }
              const px = evalTx(style.transform);
              const want = -i * (W + G);
              if (!(Math.abs(px - want) < 1e-9)) {
                bad.push(`flag=${flag} i=${i} dx=${dx} → ${k} ${style.transform} = ${px}px 期待 ${want}px`);
              }
            }
          }
        }
      ...
      // 【F-107】**ページ間の間隔は、静止時もドラッグ中も同じ。**
      // 隣り合う2ページの左端の差が常に W + G(= 溝 G)であることを、
      // index(0..3) × dx(引きの途中の全域)の総当たりで確かめる。
      // これが「指で引いている最中だけ間隔が詰まる」という症状の否定そのもの。
      {
        const bad = [];
        let n = 0;
        const N = 4;
        for (let i = 0; i < N; i++) {
          for (const dx of [-300, -150, -75, -8, -0.5, 0, 0.5, 8, 75, 150, 300]) {
            n++;
            const tx = evalTx(api.swipePagerTrackTransform(i, dx));
            if (!Number.isFinite(tx)) { bad.push(`i=${i} dx=${dx} → 読めない式`); continue; }
            // ページ p の左端(viewport 座標) = p*(W+G) + tx。隣との差は常に W+G のはず。
            for (let p = 0; p + 1 < N; p++) {
              const left = p * (W + G) + tx;
              const next = (p + 1) * (W + G) + tx;
              const gapPx = next - (left + W);
              if (!(Math.abs(gapPx - G) < 1e-9)) bad.push(`i=${i} dx=${dx} p=${p} → 溝 ${gapPx}px`);
            }
          }
        }
        check("F-107: 溝の総当たりが空回りしていない", n === 44, `${n}通り`);
        check("F-107: ドラッグ中(dx≠0)と静止時(dx=0)で、隣り合うページの間隔がどちらも溝 G と等しい",
          bad.length === 0, bad.slice(0, 3).join(" | ") || `全 ${n} 通りで溝 ${G}px`);
        check("F-107: 静止時は index 番のページの左端がちょうど 0(溝の分ずれない)",
          [0, 1, 2, 3].every((i) => Math.abs(i * (W + G) + evalTx(api.swipePagerTrackTransform(i, 0))) < 1e-9),
          [0, 1, 2, 3].map((i) => (i * (W + G) + evalTx(api.swipePagerTrackTransform(i, 0))).toFixed(3)).join(","));
        check("F-107: ドラッグ中の位置は静止時からちょうど dx だけ動く(抵抗・しきい値の規則は不変)",
          [-300, -8, 0, 8, 300].every((dx) => [0, 2].every((i) =>
            Math.abs((evalTx(api.swipePagerTrackTransform(i, dx)) - evalTx(api.swipePagerTrackTransform(i, 0))) - dx) < 1e-9)));
        // 溝は「式」と「flex の gap」の**両方**が同じトークンから来ていること。
        // 片方だけ変えると、静止時とドラッグ中でまた間隔が食い違う(F-107 の症状に戻る)。
        {
          const pager = srcOfFn(src, "SwipePager");
          check("F-107: track の gap も同じトークン(SWIPE_PAGER_GUTTER)から来ている",
            /gap: SWIPE_PAGER_GUTTER,/.test(pager) && /transform: swipePagerTrackTransform\(index, 0\)/.test(pager));
          check("F-107: ドラッグ中の transform も同じ純関数が作る(溝を片方だけに入れない)",
            /track\.style\.transform = swipePagerTrackTransform\(i, dx\);/.test(pager));
          check("F-107: 溝を素の px で書いた translateX が SwipePager に残っていない",
            !/translateX\(\$\{-i \* 100\}%\)/.test(codeOf(pager))
            && !/translateX\(calc\(\$\{-i \* 100\}% \+ \$\{dx\}px\)\)/.test(codeOf(pager)));
        }
      }
      ...
      //     8px だけ横へずらして長押しが成立した場面: 元の位置(0%)へ戻ることが要件。
      {
        ...
        check("F-79a: 8px ずらして長押しが成立 → drop で先頭ページの位置(0px)へ戻す(ずれたまま残さない)",
          k === "drop" && style && evalTx(style.transform) === 0 && style.transition === EASE,
        ...
        check("F-79a: 2ページ目にいるときは 2ページ目の位置へ戻す(0 に飛ばさない。溝込みで -(W+G))",
          s1 !== null && Math.abs(evalTx(s1.transform) - -(W + G)) < 1e-9, JSON.stringify(s1));
      }
  ...
  // --- 25.6 個体詳細(正典 .backrow / .starrow / .memoline / .numrow / .bigbtn)--
  {
    ...
    // 【D-3a 2026/08/26】「数字タップで出る形」の部品は消えたので、その綴りが detail に
    // 無いことはもう構造上落ちない(恒真)。主張を**常時表示であること**そのものに絞る。
    check("D-4: 測定データのグラフは常時表示(タップで開く分岐を持たない)",
      /<NoteAxisLineChart/.test(srcOfFn(src, "MetricTabCard"))
      && !/open \? chart :/.test(srcOfFn(src, "MetricTabCard")));
    ...
    // 正典 .numrow の実寸(値19px / ラベル10.5px / 副次10.5px)
    {
      ...
      // 正典 #14b / #15a が却下側に置き、呼び手ゼロになったので**部品ごと削除した**。
      // 正典 ↔ 定数の突き合わせは残し、**いま生きている読み手**へ当て先を移す:
      // PhraseTimeline のドリルダウン(セッション詳細の録音カードで選んだフレームの詳細)。
      // N-9 の本人指示「カードと .tile の箱を廃止し、リード個体詳細の .numrow と同じ数字の列に」
      // で、この列だけが .numrow の作法を引き継いだ。
      const numrow = srcOfFn(src, "PhraseTimeline");
      check("D-3a: .numrow の寸法を読むのは PhraseTimeline のドリルダウン(当て先が空回りしていない)",
        numrow.length > 2000 && /BARE_ROW_STYLES\.numrow\./.test(numrow),
        `${numrow.length}文字`);
      ...
      check("numrow の値は正典 .numrow .v と同じ",
        rowStyles.numrow.value === parseFloat(declOf(mockCss, ".numrow .v", "font-size")),
        `実装 ${rowStyles.numrow.value} / 正典 ${declOf(mockCss, ".numrow .v", "font-size")}`);
      check("numrow のラベルは正典 .numrow .l と同じ",
        rowStyles.numrow.label === parseFloat(declOf(mockCss, ".numrow .l", "font-size")),
        `実装 ${rowStyles.numrow.label} / 正典 ${declOf(mockCss, ".numrow .l", "font-size")}`);
      check("numrow は副次の行を持つ(正典 .numrow .b がある)",
      ...
      // 【N-7 / D-3a】使い手の無い定義を残さない(L-13 の片側落ち防止)。
      // mrow 型は N-7 で廃止。unit(正典 .v small = 11px)は D-3a で読み手ゼロになり鍵ごと削除。
      // **鍵の集合そのもの**を固定する(増やしたら落ちる = 読み手の無い鍵が黙って増えない)。
      check("D-3a: BARE_ROW_STYLES の鍵は読み手のある3つだけ(mrow / unit は廃止)",
        Object.keys(rowStyles).join(",") === "numrow"
        && Object.keys(rowStyles.numrow).sort().join(",") === "label,sub,value",
        `${Object.keys(rowStyles).join(",")} / ${Object.keys(rowStyles.numrow).sort().join(",")}`);
      check("D-3a: 寸法は BARE_ROW_STYLES から引く(読み手に数値を写していない)",
        /fontSize: BARE_ROW_STYLES\.numrow\.value, fontWeight: 600/.test(numrow)
        && /fontSize: BARE_ROW_STYLES\.numrow\.label, color: "var\(--c-ink-3\)"/.test(numrow)
        && /fontSize: BARE_ROW_STYLES\.numrow\.sub,/.test(numrow));
      // 正典 .numrow .b は「値が無い列でも場所を確保する」(§6.1.5 動かない)。
      check("D-3a: 副次の行は値が無くても場所を確保する(§6.1.5 高さが揃う)",
        /minHeight: 15 \}\}>\{c\.sub \?\? " "\}/.test(numrow));
      // 【実測で踏んだ罠】flex-basis 0 のままだと、列は**同じ行で幅0まで潰れる**(実測 327/0/0/0)。
      // 折り返しの判定に使う最小幅を与えて次の行へ送る。
      check("D-3a: 列は最小幅を持つ(幅0まで潰れた列が残らない)",
      ...
      // 【D-3 / D-4 2026/08/22 本人指示で反転】bare(枠と地色の箱を持たない数字の列)を
      // 渡していた2画面はどちらも MetricTabCard へ移り、渡し手がゼロになった。
      // 【D-3a 2026/08/26】受け口(bare / rowStyle)そのものを部品ごと削除したので、
      // 主張を「渡し手が0件」から「動く側に綴りが1つも無い」へ強める。
      const bareSites = (codeOf(src).match(/^\s*bare(?:$| rowStyle=)/gm) || []).length;
      const rowStyleSites = (codeOf(src).match(/rowStyle/g) || []).length;
      check("D-3a: bare / rowStyle の受け口は動く側に1つも残っていない",
        bareSites === 0 && rowStyleSites === 0, `bare ${bareSites}箇所 / rowStyle ${rowStyleSites}箇所`);
...
console.log("\n========== 検証26: N-6 データタブ(正典 north-star-measure.html との突き合わせ) ==========");
{
  ...
  // 【N-10 2026/08/17 本人指示で書き換え】ヒーロー(.hero*)・指標行(.mrow*)・塗りボタン(.impbtn)は
  // 画面ごと無くなったので、突き合わせ先の一覧から外した(黙って消していない)。
  // 一覧まわり(.shead / .fpills / .fp / .srow)は N-6 のまま生きているので残す。
  for (const sel of [".shead", ".shead .t", ".fpills", ".fp", ".srow", ".srow .d", ".chev"]) {
  ...
  // --- 26.1 【N-10 2026/08/17 本人指示で書き換え】紺のヒーローの廃止と右上のセレクタ -------
  // 旧 26.1 は正典 north-star-measure.html の .hero(角丸22 / padding 18-20 / 44px の主数値 /
  // 評価ピル / ± の副次行)と実装を突き合わせていた。**案K の採用でヒーローそのものが無くなった**
  // ので、主張を「面が無い」「セレクタは白地の上でも同じ作法のまま」「期間9種は不変」へ
  // 置き換える(黙って消していない)。案K 側の値の突き合わせは 26.8 が持つ。
  {
    check("26.1 N-10: 紺のヒーロー(角丸22の面)が MyDataSection に無い",
      !/borderRadius: 22/.test(codeOf(myDataSection)));
    check("26.1 N-10: 紺地の上でだけ使っていた文字色(#B9C9E4 / #9DB3D6)も残っていない",
      !/#B9C9E4|#9DB3D6/.test(codeOf(myDataSection)));
    check("26.1 N-10: 主数値 44px・単位 21px・評価ピルの綴りが残っていない",
      !/fontSize: 44,/.test(codeOf(myDataSection)) && !/fontSize: 21,/.test(codeOf(myDataSection)));
    // 期間・楽器のセレクタは白地へ移ったが、作法(素のテキスト + ▾ / 44pt / 型のクラスを
    // 持たない)は F-72 のまま不変。**2つとも**見る(片方だけ直す事故を作らない)。
    // 【N-11 2026/08/17 本人指示】置き場所が MyDataSection の上部から**子タブ行の右端**
    // (MyDataScopePicker)へ移った。作法の主張はそのまま、走査先だけを移す。
    const scopePicker = srcOfFn(src, "MyDataScopePicker");
    check("26.1 N-11: 集計範囲セレクタの部品を走査できている(空回りしていない)",
      scopePicker.length > 800, `${scopePicker.length}文字`);
    check("26.1 N-11: MyDataSection に独立したセレクタの帯が残っていない(上部の空白の解消)",
      !/aria-label="集計する期間を選ぶ"/.test(myDataSection)
      && !/aria-label="集計する楽器種別を選ぶ"/.test(myDataSection));
    for (const [nm, label, tail] of [
      ["期間", "集計する期間を選ぶ", "{rangeLabel} ▾"],
      ["楽器種別", "集計する楽器種別を選ぶ", "{SAX_PRESETS[dataSax]?.label} ▾"],
    ]) {
      const i = scopePicker.indexOf(`aria-label="${label}"`);
      const tag = i === -1 ? "" : scopePicker.slice(scopePicker.lastIndexOf("<button", i), scopePicker.indexOf("</button>", i));
      check(`26.1 ${nm}セレクタのボタンを走査できている`, tag.length > 100, `${tag.length}文字`);
      check(`26.1 ${nm}セレクタは地も枠も持たない(型のクラスも持たない)`,
        /background: "none", border: "none"/.test(tag) && !/\bctl-(state|plain|pill)\b/.test(tag),
        tag.replace(/\s+/g, " ").slice(0, 200));
      check(`26.1 ${nm}セレクタには ▾ が付く(押せば選択肢が出ることを形で示す。F-72)`,
        tag.includes(tail), tag.replace(/\s+/g, " ").slice(0, 200));
      // 【実測で踏んだ罠】高さだけ 44 にしても、「昨日 ▾」のように短い値では**幅が 39px**しか
      // 無く §5 を割る(375×812 で elementFromPoint を整数グリッド1px刻みに走査)。
      check(`26.1 ${nm}セレクタの当たり判定は縦横とも 44pt(§5。短い値でも幅が足りる)`,
    ...
    // 期間の候補は9種すべて残っている(機能を落とさない)
    {
      ...
      check("26.1 期間の選択肢はシートに MY_DATA_RANGES から流し込む(綴りを2箇所に置かない)",
        /items=\{rangeOptions\.map\(\(o\) => \(\{ key: o\.key, label: o\.label \}\)\)\}/.test(scopePicker));
    }
    // 【N-11】上部の空白の解消。集計範囲セレクタは独立した行(帯)を作らず、既にある行へ相乗りする。
    // 【D-9z / D-9r 2026/08/25 本人指示で乗せ先が変わった】相乗り先が**子タブ行**から
    // **My Data の指標タブの行**へ下りた(本人「このセクタは mydata にしか影響しないのに、
    // mydata・分析と同じ位置にあるのが違和感」)。子タブ行は素のテキスト2つだけになる。
    {
      const iRow = lab.indexOf('{ key: "mydata", label: "My Data" }');
      const iPager = lab.indexOf("<SwipePager", iRow);
      const row = iRow > 0 && iPager > iRow ? lab.slice(iRow, iPager) : "";
      check("26.1 子タブ行を走査できている(空回りしていない)", row.length > 100, `${row.length}文字`);
      check("26.1 D-9z: 集計範囲セレクタは子タブ行から出ている(右端は両タブとも空)",
        !/<MyDataScopePicker/.test(row), row.includes("MyDataScopePicker") ? "行の中" : "行の外");
      check("26.1 D-9z: セレクタは My Data の指標タブの行へ相乗りする(独立した帯を作らない)",
        /<MetricUnderlineTabs[\s\S]{0,400}?<MyDataScopePicker/.test(myDataSection)
        && /marginLeft: "auto"/.test(scopePicker));
      // 【D-9r】子タブ行に「My Data のときだけ何かが生える」分岐がもう無い
      // (右端が両タブとも空になったので、この行から dataSubTab の分岐が1つ消えた)。
  ...
  // **「面はマトリクス2枚 + 練習カード1枚だけ」**を新しく足す。中身は検証27。
  {
    ...
    // 上下2枚(今日の自分 / いつもの自分)をやめ、**選んだ2つの系列の引き算1枚**になった。
    check("26.2 D-9: マトリクスの呼び出しは1つだけ(グラフは1つに固定)",
      (myDataSection.match(/<NoteMatrixBlock/g) || []).length === 1
      && (src.match(/<NoteMatrixBlock/g) || []).length === 1,
    ...
    // 【D-3a 2026/08/26】タップ切り替えの部品そのものが消えたので、綴りの不在は恒真になった。
    // 主張を「マトリクスは開閉の状態を持たない(常に出ている)」へ移す。
    check("26.2 D-1: マトリクスは開閉を持たず常に出ている",
      !/useState\(false\)/.test(srcOfFn(src, "NoteMatrixBlock")));
    ...
    // グラフ部品(NoteAxisLineChart)の目安の仕組みそのものは
    // **他画面(リード詳細・セッション詳細)のために従来のまま**であることを固定する。
    {
      ...
      // (【D-3a 2026/08/26】「TappableMetricCard が idealDiffText をそのまま渡す」検査は
      //  ここにあったが、部品ごと削除したので落とした。受け取る側(グラフ部品)が
      //  仕組みを保っていることは、すぐ上の検査が引き続き固定している。)
    }
  }
  ...
  // --- 26.3 セッション一覧(正典 .shead / .fpills / .srow) ----------------------
  {
    ...
    // **値は正典 .srow のまま 1px も動かしていない**ことを、CSS 側と突き合わせて確かめる。
    {
      const rowCss = (/\.slist-row \{([^}]*)\}/.exec(cssIdx) || ["", ""])[1];
      const dateCss = (/\.slist-date \{([^}]*)\}/.exec(cssIdx) || ["", ""])[1];
      const fsSm = (/--fs-sm:\s*(\d+)px/.exec(cssIdx) || [])[1];
      check("26.3 index.css の一覧の行の規則を読めている(空回りしていない)",
        rowCss.length > 40 && dateCss.length > 10 && fsSm !== undefined,
        `row=${rowCss.length}字 / date=${dateCss.length}字 / --fs-sm=${fsSm}`);
      check("26.3 一覧の行の余白は正典 .srow と同じ",
        new RegExp(`padding:\\s*${declOf(mockCss, ".srow", "padding")}\\s*;`).test(rowCss),
        `正典=${declOf(mockCss, ".srow", "padding")} / 実装=${rowCss.replace(/\s+/g, " ").trim()}`);
      check("26.3 一覧の行の gap も正典 .srow と同じ",
        new RegExp(`gap:\\s*${declOf(mockCss, ".srow", "gap")}\\s*;`).test(rowCss),
        `正典=${declOf(mockCss, ".srow", "gap")}`);
      check("26.3 一覧の行の日時は正典 .srow .d と同じ大きさ(--fs-sm 経由)",
        /font-size:\s*var\(--fs-sm\)\s*;/.test(dateCss)
        && Number(fsSm) === parseFloat(declOf(mockCss, ".srow .d", "font-size")),
        `正典=${declOf(mockCss, ".srow .d", "font-size")} / --fs-sm=${fsSm}`);
    }
    ...
    // 【N-10 / F-108】見出しの右は正典 案K の .ops =「↥ 取り込み」「選択」の2つ。
    {
      const opsBlock = /<span class="ops">([\s\S]*?)<\/span><\/div>/.exec(kBlock)
        || /<span class="ops">([\s\S]*?)<\/div>/.exec(kBlock);
      const ops = opsBlock ? [...opsBlock[1].matchAll(/<span>([^<]+)<\/span>/g)].map((x) => x[1]) : [];
  ...
  {
    // 【N-10】録音長そのものの導出は sessionDurationSec の1箇所へ寄せた
    // (一覧の m:ss と My Data の総演奏時間が同じ規則から出る)。表示の形だけが別関数。
  ...
  // 旧 26.6 は「押せる項目が無ければ「…」を出さない」(F-77 の罠)を dataMoreItems の実行で
  // 固定していた。本人指示「セッションの削除についての機能なのに一覧との距離が遠くて使いづらい」
  // により、中身の2項目(削除 / 一括リード変更)は**どちらも一覧の見出しの「選択」の中へ入り**、
  // 「…」は中身が空になったので**ボタンごと出さなくなった**(F106-SPEC「空なら「…」自体を出さない」)。
  // したがって主張を「「…」がもう無い」「機能は落ちていない」へ反転させる(黙って消していない)。
  {
    const code = codeOf(src);
    ...
    // 入口が1つになったので、モードの値も1つ("select")。入口・出口・実行の3本が同じ値を見る。
    {
      ...
      check("26.6 F-108: セッションの選択モードは1つだけ(N-6 の delete / reed の2値は統合された)",
        mode === "select" && !/listMode === "delete"|listMode === "reed"/.test(code), String(mode));
      // 【N-11 2026/08/17 本人指示】一括リード変更を消したので setBulkReedId の1行が消えた。
      // 主張(「その1つの値だけを立て、選択状態を空から始める」)は不変で、錨だけ縮む。
      check("26.6 F-108: 入口(startListMode)はその1つの値だけを立て、選択状態を空から始める",
        /const startListMode = \(\) => \{[\s\S]{0,160}?setSelectedForDelete\(new Set\(\)\);[\s\S]{0,80}?setListMode\(SESSION_SELECT_MODE\);[\s\S]{0,12}?\};/.test(lab));
      check("26.6 F-108: その入口を一覧(MyDataPage)へ渡している(呼び出しに隣接する綴り)",
        /onStartSelect=\{startListMode\}/.test(lab));
    }
  ...
  // 正典と現行がぶつかったら**機能を残す**(2026/08/12 本人確定)。形が変わっても消さないもの。
  {
    ...
    const want = [
      ...
      ["系列の切り替え(式の左右)", /onClick=\{\(\) => setSheetSide\(side\)\}/],
      ...
      ["式の記号(× / ー)", /MY_DATA_EXPR_OPERATORS\[view\]/],
      ...
      //  本人が名指しで削除を指示した唯一の機能で、27.7 が「もう無い」ことを固定する。)
...
console.log("\n========== 検証27: D-1 My Data(正典 dc-mydata-redesign.html の #9b) ==========");
{
  ...
  // --- 27.1 高い/低いの2色(D-1 の8段の発散スケールは廃止) ----------------------
  {
    ...
    check("27.1 D-8: 帯の中の窓は地を --c-sunk・字を --c-ink-3 で引っ込める",
      /return \{ bg: "var\(--c-sunk\)", fg: "var\(--c-ink-3\)", step: 0 \};/.test(src));
    // 【D-9 §5 2026/08/25 本人確認済み】折れ線の「合っている」帯は**出さない**
    // (本人「人間の感覚的に食い違いません」)。窓型の引っ込め(上の --c-sunk)は残る。
    check("27.1 D-9 §5: 折れ線に「合っている」帯を塗らない(濃さの定数ごと消えている)",
      !/CHART_OK_BAND_OPACITY/.test(codeOf(src))
      && !/fill: "var\(--c-quiet\)"/.test(codeOf(src)),
      "");
  ...
  // ただし**帯(合っている幅)は残す**(本人裁定)ので、帯の中の窓は段に乗せず灰へ引っ込める。
  {
    ...
    // --- 27.2d 中央線の値(D-9u。折れ線は差分をやめ、2本をそのまま重ねる形になった) ---
    // 【D-9u 2026/08/25 本人指示】「平均差分以外はその項目の平均を中央線にして同様に」。
    // 期待値は**実装の式で書き直さない**(罠3)。平均差分は 0 という**約束**そのものを見て、
    // 他の3指標は手で足した平均(下の 30 は (10+20+30+60)/4 = 30)と突き合わせる。
    {
      const cv = api.myDataCenterValue;
      check("27.2d D-9u: 平均差分の中央線は 0(母集団で動かない)",
        cv("pitchCentsSigned", [{ 0: 5, 1: -3 }]) === 0 && cv("pitchCentsSigned", [{}]) === 0,
        String(cv("pitchCentsSigned", [{ 0: 5, 1: -3 }])));
      check("27.2d D-9u: 他の3指標は**描いている全値**の平均(系列をまたいで1つ)",
        cv("hnrDb", [{ 0: 10, 1: 20 }, { 0: 30, 1: 60 }]) === 30,
        String(cv("hnrDb", [{ 0: 10, 1: 20 }, { 0: 30, 1: 60 }])));
      check("27.2d D-9u: 描いていない系列・読めない音は母集団に入らない",
        cv("hnrDb", [{ 0: 10, 1: null, 2: undefined, 3: NaN }, {}]) === 10,
        String(cv("hnrDb", [{ 0: 10, 1: null, 2: undefined, 3: NaN }, {}])));
      check("27.2d D-9u: 値が1つも無ければ null(中央線を引かない = 0 と偽らない)",
        cv("hnrDb", [{}, {}]) === null && cv("hnrDb", null) === null,
        String(cv("hnrDb", [{}, {}])));
    }
  ...
  // --- 27.3 マトリクスの組み立て(実行で検証) --------------------------------------
  {
    const api = new Function(`${extractConst("NOTE_NAMES")}
      ...
      ${extractFunction("getNoteIdeal")}
      ${extractFunction("myDataSeriesByIdx")}
      const concertNoteLabelOf = (i) => LABELS[i] ?? null;
      let LABELS = [];
      ${extractFunction("buildNoteMatrix")}
      return { buildNoteMatrix, myDataSeriesByIdx, noteGroupKeyOf, NOTE_NAMES,
      ...
      api.noteGroupKeyOf("spectralCentroidHz"));
    {
      ...
      // 【D-9 §4 案D】空セルを2つに分けるための土台。**軸にある音**を値とは別に持つ。
      check("27.3 D-9 案D: 音域の中の窓は値の有無に関わらず inRange に入る",
        m.inRange["3:C"] === true && m.inRange["3:B"] === true && m.inRange["4:C"] === true,
        Object.keys(m.inRange).join(","));
      check("27.3 D-9 案D: 軸に無い音は inRange に入らない(窓を置かない側)",
        m.inRange["3:F"] === undefined && m.inRange["5:C"] === undefined
        && m.inRange["4:G♯"] === undefined,   // count=5 なので G♯4 は軸の外
        Object.keys(m.inRange).join(","));
    ...
    }
    {
      ...
      check("27.3 実測か比較対象のどちらかが読めない音はセルごと空(0 で埋めない)",
        ...
        JSON.stringify(m.byKey));
      // 【D-9 §4 案D】値が読めない音でも、**軸にある**なら inRange には残る
      // (だから「まだ弾いていない」として枠だけの窓を置ける)。
      check("27.3 D-9 案D: 値が読めない音も軸にあれば inRange に残る(枠だけの窓になる)",
        m.inRange["3:D"] === true && m.inRange["3:E"] === true,
        Object.keys(m.inRange).join(","));
    ...
    }
    {
      ...
      const f = (k, m) => api.myDataSeriesByIdx(k, day, period, ideal, m ?? "pitchCentsSigned", 2);
      check("27.3 D-9: その日は当日(または直近日)の値をそのまま返す(別の集計を作らない)",
      ...
      check("27.3 D-9: 目安の重心は centroidHz を引く(綴りの対応づけを通す)",
        JSON.stringify(f("reference", "spectralCentroidHz")) === '{"0":900}');
    }
  }
  ...
  // --- 27.4b 折れ線と窓型の切替 / 引き算の置き場所(実行 + 配線で検証) ---------------
  // 【D-7 2026/08/23 本人指示】「mydata は窓型と折れ線グラフ二つ選んで見れるように。
  // デフォルトは折れ線グラフで」。
  // 【D-9 §1 2026/08/25 本人指示で書き換え】2つの見せ方は**同じ2つの系列**を見る。
  // 折れ線は2本を重ね(×)、窓型はその2本を引く(ー)。**引き算をするのは窓型だけ**になり、
  // 折れ線が持っていた差分の導出(noteDiffByIdx)は要らなくなった。
  {
    ...
    const api = new Function(`const Activity = "Activity", Grid3x3 = "Grid3x3";
      ...
      ${extractConst("MY_DATA_EXPR_OPERATORS")}
      return { MY_DATA_VIEWS, MY_DATA_VIEW_DEFAULT, MY_DATA_EXPR_OPERATORS };`)();
    ...
    // 【D-9 §1】記号の使い分けは意図的(本人確認済み)。× = 2本を重ねる / ー = 引き算。
    check("27.4b D-9: 記号は見せ方ごとに1つ(折れ線 × / 窓型 ー)",
      api.MY_DATA_EXPR_OPERATORS.line === "×" && api.MY_DATA_EXPR_OPERATORS.matrix === "ー"
      && Object.keys(api.MY_DATA_EXPR_OPERATORS).sort().join(",") === "line,matrix",
      `${api.MY_DATA_EXPR_OPERATORS.line} / ${api.MY_DATA_EXPR_OPERATORS.matrix}`);
    check("27.4b D-9: 記号は見せ方の key から引く(画面に × / ー を直書きしない)",
      /MY_DATA_EXPR_OPERATORS\[view\]/.test(myDataSection));

    // 【D-9 の芯】引き算は**窓型の1箇所(buildNoteMatrix)だけ**。
    // 折れ線は系列をそのまま描くので、引き算が2箇所に増えていないことをここで固定する。
    {
      const two = new Function(`${extractConst("NOTE_NAMES")}
        ...
        const concertNoteLabelOf = (i) => LABELS[i] ?? null;
        ${extractFunction("buildNoteMatrix")}
        return { buildNoteMatrix, parseNoteLabel, setLabels: (a) => { LABELS = a; } };`)();
      const LBL = ["C3", "C♯3", "D3", "B3", "C4", "G♯4"];
      two.setLabels(LBL);
      const a = { 0: 10, 1: -4, 2: 0, 3: 2, 4: 6, 5: -7 };
      const b = { 0: 4, 1: -1, 2: 0, 3: 5, 4: 6 };   // 5 は2本目が読めない音
      const mtx = two.buildNoteMatrix(a, b, 6, null, null);
      // 期待値は**実装の式ではなく引き算そのもの**から作る(定義の言い換えを書かない)。
      const mismatches = [];
      for (let i = 0; i < 6; i++) {
        const p = two.parseNoteLabel(LBL[i]);
        const got = p ? mtx.byKey[Number(p.octave) + ":" + p.name] : undefined;
        const want = (b[i] === undefined || b[i] === null) ? undefined : a[i] - b[i];
        const same = (want === undefined) ? (got === undefined) : Math.abs(got - want) < 1e-9;
        if (!same) mismatches.push(`${i}: 窓=${got} / 期待=${want}`);
      }
      check("27.4b D-9: 窓型が描くのは「1本目 − 2本目」そのもの",
        mismatches.length === 0, mismatches.join(" / ") || "全音一致");
      check("27.4b D-9: 突き合わせが空回りしていない(実際に値が入っている)",
        Object.keys(mtx.byKey).length === 5 && mtx.byKey["3:C"] === 6,
        `${Object.keys(mtx.byKey).length}音 / 3:C=${mtx.byKey["3:C"]}`);
      check("27.4b D-9: 2本目が読めない音は窓を持たない(0 で埋めない)",
        mtx.byKey["4:G♯"] === undefined, String(mtx.byKey["4:G♯"]));
      // 折れ線は**引かずにそのまま**渡す。引き算の綴りが折れ線側に生えたら落ちる。
      check("27.4b D-9: 折れ線には系列をそのまま渡す(引き算をもう1箇所に書かない)",
        /byIdx: seriesA/.test(myDataSection) && /byIdx: seriesB/.test(myDataSection)
        && !/noteDiffByIdx/.test(codeOf(src)),
        "");
    }
  }
  ...
  // --- 27.4c 配線: 式の行・切替・シート ---------------------------------------------
  {
    ...
    // 【D-9 §3 2026/08/25 本人指示】この行だけ §5(44pt)の例外。本人「min高さが44に
    // 設定されていましたが、この行で44は余白が大きすぎるのでここは最小44の必要はありません」。
    // 切替は式の行に同居するので、**行と同じ高さ**でなければ行が 44 に戻ってしまう。
    check("27.4c D-9 §3: 切替の高さは式の行と同じ(44 の床を持たない)",
      /height: MY_DATA_EXPR_ROW_H, minWidth: MY_DATA_EXPR_ROW_H/.test(vt)
      && !/var\(--tap-min\)/.test(vt),
      "");
    ...
    check("27.4c 切替は式の行の右端に同居する(行を増やさない)",
    ...
    check("27.4c D-9 §2: 凡例(legend)は部品からも配線からも消えている",
      ...
      && !/legend/.test(codeOf(myDataSection)),
      "");
    // 【D-9 §3】見た目 20px / 当たり判定は行の高さいっぱい。**両方**が要る
    // (見た目だけ小さくして当たり判定も 20 にすると押しにくい画面になる)。
    check("27.4c D-9 §3: チップの見た目は 20px、当たり判定は行の高さいっぱい",
      /height: MY_DATA_EXPR_CHIP_H/.test(myDataSection)
      && /height: MY_DATA_EXPR_ROW_H, display: "inline-flex"/.test(myDataSection),
      "");

    // 【D-9 で 1 → 2】`view === "line" ?` は式のチップの枠にも出るようになったので、
    // 件数ではなく**本体の分岐そのもの**(折れ線 ⇄ 窓型)を名指しで縛る。
    check("27.4c D-8: 本体は見せ方で出し分ける(折れ線 / 窓型)",
      /\{view === "line" \? \(\s*\r?\n\s*<NoteAxisLineChart/.test(myDataSection)
      && /\) : \(\s*\r?\n\s*<NoteMatrixBlock/.test(myDataSection),
      "");
  ...
  // 純関数が正しくても、描画側が正しい引数で呼んでいなければ画面には出ない(罠2)。
  {
    ...
    // 【D-9 §1】式の左右は**同じ1関数**から引く(左右で母集団の作り方が違う、を作らない)。
    check("27.7 D-9: 式の左右は myDataSeriesByIdx の1関数から引く(片側だけ別の作り方にしない)",
      /const seriesA = myDataSeriesByIdx\(pair\[0\], dayByIdx, periodByIdx, selectedIdeal, chartMetric\.key, noteCount\);/.test(myDataSection)
      && /const seriesB = myDataSeriesByIdx\(pair\[1\], dayByIdx, periodByIdx, selectedIdeal, chartMetric\.key, noteCount\);/.test(myDataSection)
      && (codeOf(myDataSection).match(/myDataSeriesByIdx\(/g) || []).length === 2,
      `${(codeOf(myDataSection).match(/myDataSeriesByIdx\(/g) || []).length}箇所`);
    check("27.7 D-9: 窓型は1枚。その2つの系列を引く(グラフは1つに固定)",
      /const matrix = buildNoteMatrix\(seriesA, seriesB, noteCount, dataSax, tuningHz\);/.test(myDataSection)
    ...
    check("27.7 D-9: 選べない系列が選ばれたまま残らない(状態そのものではなく落とし先を使う)",
      ...
      `${(codeOf(myDataSection).match(/pairRaw/g) || []).length}箇所`);
    // 【D-9u】中央線は**描いている2本**から出す(母集団を取り違えると絵が変わる)。
    check("27.7 D-9u: 中央線は描いている2本の系列から出す(別の母集団で作らない)",
      /const centerAt = myDataCenterValue\(chartMetric\.key, \[seriesA, seriesB\]\);/.test(myDataSection)
      && /centerAt=\{centerAt\}/.test(myDataSection));
  ...
  // --- 27.9 消えた側の綴りが残っていない(読み手ゼロの定義を残さない) ---------------
  {
    ...
    // 【D-9 / D-3a 2026/08/26 で追加】D-9 の裁定で読み手ゼロになった定義をこの集合へ足す:
    //   ・noteDiffByIdx / myDataLineIsDiff … 折れ線が差分をやめ、引き算は窓型だけになった
    //   ・compareTargetByIdx / myDataCompare* … 「1つ選ぶ」前提の3つは式の2系列へ作り替えた
    //   ・matrixBandText … D-8 で凡例が消えたときから読み手ゼロだった
    //   ・TappableMetricCard … 呼び手ゼロ(D-3a)
    const dead = ["myDataChartSeries", "noteSpreadByIndex", "metricCardNumbers",
      ...
      "noteDiffByIdx", "myDataLineIsDiff", "compareTargetByIdx", "myDataCompareTargets",
      "myDataCompareDefault", "myDataCompareFallback", "matrixBandText", "TappableMetricCard"];
    ...
    // 【D-9 2026/08/25 本人指示】D-8 の折れ線(ピッチだけ差分 / 実数のときは比較対象を破線 /
    // 凡例)は**すべて置き換わった**。いまの折れ線は「選んだ2本をそのまま重ねる」形で、
    // 基準は中央線1本(D-9u)、名前はチップの枠の色(§2)が担う。
    check("27.9 D-9: 折れ線には myData プリセットと centerAt だけを渡す(受け口を8つに増やさない)",
      /<NoteAxisLineChart\s*\r?\n\s*myData\s*\r?\n/.test(myDataSection)
      && !/plain legend axisOverlay/.test(myDataSection)
      && !/zeroCentered|bandAbs|refByIdx|refLabel/.test(codeOf(src)),
      "");
    check("27.9 D-9: 消えた受け口は部品の側にも残っていない(読み手ゼロの分岐を残さない)",
      !/legend/.test(codeOf(srcOfFn(src, "NoteAxisLineChart")).replace(/legendMax|legendPad/g, ""))
      && !/axisOverlay/.test(codeOf(src)),
      "");
    // 【変異試験 M28(D-8)で生存 → 足した検査を引き継ぐ】重ねた軸ラベルの白い縁を外すと
    // 折れ線に埋もれて読めない(重ねる方式の代償を打ち消す仕掛けそのもの)。
    check("27.9 D-9: 重ねた軸ラベルは白い縁で折れ線から浮かせる(重ねない画面には付けない)",
    ...
    check("27.9 D-9v: My Data では上下の水平線を引かず、目盛の文字だけ残す",
      ...
      && /\{L\.tickTexts\[k\]\}/.test(srcOfFn(src, "NoteAxisLineChart")));
    // 【D-9w】作図域の高さ。My Data だけ 170(他の3画面は 94 のまま)。
    check("27.9 D-9w: 作図域は My Data だけ高い(他画面は従来のまま)",
      /const plotH = myData \? CHART_PLOT_H_MY_DATA : CHART_PLOT_H;/.test(srcOfFn(src, "NoteAxisLineChart"))
      && /const CHART_PLOT_H = 94;/.test(src) && /const CHART_PLOT_H_MY_DATA = 170;/.test(src));
    // 【D-9 §3 の余白】式の行は 35px。本人が2周続けて「縦幅が過剰」と言った箇所なので、
    // **削った高さ**を綴りで固定する(戻すと落ちる)。
    check("27.9 D-9 §3: 式の行の高さは 35px(44 の床を持たない)",
      /height: MY_DATA_EXPR_ROW_H \}\}>/.test(myDataSection)
      && /const MY_DATA_EXPR_ROW_H = 35;/.test(src) && /const MY_DATA_EXPR_CHIP_H = 20;/.test(src));
...
// ============================================================
// 検証28: N-8 直近日フォールバック + 目安の撤去
//   (2026/08/16 本人指示。凍結仕様 = design/N8-SPEC.md)
//   A 直近日フォールバック myDataTodayOrLatestFrames(抽出して実行で検証)
//   B 描画側の配線: 戻り値(frames / label)を数字カード・濃い線・凡例の**すべて**が使う
//     — 「純関数は完璧・配線が無防備」で通算17回差し戻された形を、名指しで固定する
//   C 目安(selectedIdeal)の破線と Δ が My Data の全グラフから外れている(他画面はそのまま)
// 【N-10 2026/08/17 本人指示で書き換え】旧 28.3(ヒーローの中の折れ線の hero 変形と、
// 正典 data-tab-final.html のヒーローの svg との色の突き合わせ)は、紺のヒーローが
// **画面ごと無くなった**ので削除した(黙って消していない)。正典もこの節から離れ、
// 案K との突き合わせは検証27 が持つ。ここに残るのは正典に依存しない A / B / C だけ。
// ============================================================
console.log("\n========== 検証28: N-8 直近日フォールバック + 目安の撤去 ==========");
{
  const myDataSection = srcOfFn(src, "MyDataSection");
...
console.log("\n========== 検証29: N-9 セッション詳細 + 分析(PIVOT)の北極星化 ==========");
{
  ...
  // 【D-9r 2026/08/25 本人指示で反転】「PIVOT の文字ごと削除してその分上に詰めましょう」。
  // F-99 で復活させた表題は**本人の裁定で消えた**(見出しは子タブの「分析」1つだけ)。
  // 説明の1行は F-99 の意図どおり残るので、下の検査がそちらを引き続き固定する。
  check("29.1 D-9r: 表題「PIVOT」は文字ごと消えている(見出しは子タブの1つだけ)",
    !/>\s*PIVOT\s*</.test(codeOf(src))
    && !/fontSize: 15, color: "var\(--c-accent\)", fontWeight: 700/.test(codeOf(lab29)),
...
// ============================================================
// 検証30: 実機フィードバック(第3陣 F-97〜F-104 = design/F97-SPEC.md /
//          第4陣 F-106 = design/F106-SPEC.md。いずれも本人指示)
// 【N-10 の周で 30.1 を全面書き換え】F-97 の対処(常設 checkbox + 行ごとの display 切替)は
// **実機で直らなかった**ので、F-106 で「祖先クラス1つ」へ仕様が転換した。30.2〜30.4 は不変。
...
// この節はその証明ではない(名乗りに注意)。
// ============================================================
console.log("\n========== 検証30: 実機フィードバック(F-97〜F-104 / F-106) ==========");
{
  // --- 30.1 F-106: 選択モードは**祖先クラス1つ**だけで表現する ----------------------
  // 【F-106 2026/08/17 本人指示で全面書き換え。凍結仕様 = design/F106-SPEC.md】
  // 本人実機報告: 削除モードで先頭行だけチェックボックスが出ず、その行をタップすると
  // チェックが日付に重なる(iPhone)。**F-97 の対処(常設 checkbox + 行ごとの display 切替)でも
  // 実機では直らなかった**ので、仕様が「行ごとに要素を出し入れするのをやめる」へ転換した。
  //   ・チェックボックスは常に DOM にあり、**常にレイアウトに参加する**
  //   ・モードの切替は祖先(一覧のコンテナ)のクラス1つだけ
  //   ・行の寸法・字下げ・見え隠れは index.css が持つ(行に style 属性を置かない)
  //   ・モード外は opacity:0 + pointer-events:none。**レイアウトは動かさない**
  //   ・一覧の行にもスクロールコンテナにも contain / content-visibility / will-change /
  //     transform を置かない
  // 旧 30.1(display 切替 / tabIndex / aria-hidden の綴り)は**その形ごと否定された**ので、
  // 主張も反転させる(黙って消していない)。
  ...
  // 固定できるのは「モードで行ごとの DOM/style が動く形が構造的に作れないこと」だけ。
  {
    ...
    {
      const hits = [...rows.matchAll(/listMode/g)].map((m) =>
        rows.slice(Math.max(0, m.index - 60), m.index + 20).replace(/\s+/g, " "));
      const outsideOnClick = hits.filter((h) => !/className=\{listMode \?/.test(h) && !/onClick=\{\(\) => \(listMode/.test(h));
      check("30.1 F-106: 行の中で listMode を見るのは行タップの行き先だけ(style/属性は見ない)",
        outsideOnClick.length === 0, outsideOnClick.slice(0, 2).join(" | ") || `${hits.length}箇所すべて許容形`);
    }
    // (c) 行にも checkbox にも **style 属性が無い**(行ごとの style がモードで動きようがない)
    check("30.1 F-106: 行の <div> に style 属性が無い(寸法は index.css の .slist-row が持つ)",
      /<div\s*\r?\n\s*key=\{s\.id\}\s*\r?\n\s*onClick=[^\n]*\s*\r?\n\s*className="sans slist-row"\s*\r?\n\s*>/.test(rows));
    check("30.1 F-106: チェックボックスに style 属性が無い(見え隠れは index.css が持つ)",
      /type="checkbox"[\s\S]{0,300}?className="slist-check"\s*\r?\n\s*\/>/.test(rows)
      && !/type="checkbox"[\s\S]{0,300}?style=\{\{/.test(rows));
    // (d) 旧形(display 切替 / 条件マウント / tabIndex / aria-hidden のモード分岐)が残っていない
    check("30.1 F-106: display の切替が残っていない(F-97 の形へ戻せない)",
      !/display: listMode/.test(codeOf(page30)));
    check("30.1 F-106: 条件マウント(listMode && <input)も残っていない",
      !/listMode && \(\s*\r?\n\s*<input/.test(codeOf(page30)));
    check("30.1 F-106: 行ごとの tabIndex / aria-hidden のモード分岐が残っていない",
      !/tabIndex=\{listMode/.test(page30) && !/aria-hidden=\{listMode/.test(page30));
    // (e) index.css 側: レイアウトを動かさずに見え隠れさせる / 祖先クラスだけが切り替える
    {
      // 宣言だけを見る。**コメントを剥がしてから**判定しないと、経緯を書いたコメントの中の
      // 語(例「display:none には無かった劣化」)を宣言と誤認する(実際にそれで落ちた)。
      const chk = (/\.slist-check \{([^}]*)\}/.exec(css30) || ["", ""])[1].replace(/\/\*[\s\S]*?\*\//g, "");
      const on = /\.slist\.is-select \.slist-check \{([^}]*)\}/.exec(css30);
      // 【審査で訂正 2026/08/17】初版は opacity:0 を要求し visibility を**禁じて**いたが、
      // opacity は読み上げとフォーカス順を殺さず、モード外の不可視チェックボックスが
      // tabbable に残っていた(実測: ページの tabbable 36個中14個)。
      // visibility は display と違い**レイアウトを 1px も動かさない**ので、F-106 の不変条件
      // (祖先クラス1つ・行ごとの更新ゼロ・レイアウト不動)を崩さずに劣化だけ消える。
      // **display は引き続き禁止**(F-97 の失敗そのもの。レイアウトが動く)。
      check("30.1 F-106: モード外は visibility:hidden + pointer-events:none(display は切らない)",
        /visibility:\s*hidden\s*;/.test(chk) && /pointer-events:\s*none\s*;/.test(chk)
        && !/display:/.test(chk) && !/opacity:/.test(chk), chk.replace(/\s+/g, " ").trim());
      check("30.1 F-106: モード内は visibility:visible へ戻す(祖先クラス側だけで切り替える)",
        on !== null && /visibility:\s*visible\s*;/.test(on[1]) && /pointer-events:\s*auto\s*;/.test(on[1]),
        on ? on[1].replace(/\s+/g, " ").trim() : "規則が無い");
      check("30.1 F-106: チェックボックスは常に場所を取る(幅は 0 にしない)",
        /flex:\s*0 0 20px\s*;/.test(chk) && /width:\s*20px\s*;/.test(chk), chk.replace(/\s+/g, " ").trim());
      // 【審査で訂正 2026/08/17】visibility へ移したので、禁じる側から visibility を外し
      // opacity を入れる。**レイアウトを動かす宣言(width/height/display/margin/padding/flex)は
      // 引き続き全部禁止** — ここが F-106 の芯(祖先クラス1つで、レイアウトは1pxも動かさない)。
      check("30.1 F-106: モード内で変わるのは visibility と pointer-events だけ(レイアウトは動かない)",
        on !== null && /visibility:\s*visible\s*;/.test(on[1]) && /pointer-events:\s*auto\s*;/.test(on[1])
    ...
    //     iOS の再ペイント取りこぼしの典型要因(F106-SPEC が名指し)。
    check("30.1 F-106: SwipePager の track に will-change が残っていない(祖先のレイヤ昇格を外した)",
      !/willChange/.test(codeOf(srcOfFn(src, "SwipePager"))));
    check("30.1 F-106: アプリ全体で will-change を1つも使っていない(SwipeBackArea の作法に統一)",
      !/willChange|will-change:/.test(codeOf(src)) && !/will-change\s*:/.test(css30));
  }
...
// ============================================================
// 検証31: N-11 My Data グラフカード + F-111 浮かせるボタン
//   (2026/08/17 本人指示。凍結仕様 = design/N11-SPEC.md)
//   正典 = design/mydata-v2-proposals.html の**案P**(グラフカードの地だけ同ファイルの案R)
//        + design/detail-and-reed-proposals.html の**案D**(浮かせるボタン)
//   31.1 グラフカード(地・角丸・画面いっぱい)
//   31.2 Y軸のラベルをプロットへ重ねる(線の描画幅が広がる)
//   31.3 My Data のグラフだけフラジオを含めない(他画面は含めたまま。両方向を固定)
//   31.4 浮かせるボタンは2画面で**同じ1部品**(作法を2つ作らない)
// 【この節の作り方】ハーネスは JSX を評価しないので、数と音域は**抽出して実行**で検証し、
...
console.log("\n========== 検証31: F-111 浮かせるボタン(N-11 のグラフカードは D-1 で撤去) ==========");
{
  ...
  const tok31 = (name) => (new RegExp(`${name}:\\s*(#[0-9A-Fa-f]{6})`).exec(cssIdx31) || [])[1];
  const myDataSection = srcOfFn(src, "MyDataSection");
  const chart = srcOfFn(src, "NoteAxisLineChart");
  const pager = srcOfFn(src, "SwipePager");
  const fab = srcOfFn(src, "FloatingAction");

  // 正典の CSS 規則を読む(コメントを外してから)
  const pCss = (pMock.match(/<style>([\s\S]*?)<\/style>/) || ["", ""])[1].replace(/\/\*[\s\S]*?\*\//g, " ");
  const pDecl = (sel, name) => {
    const m = new RegExp(`(^|[};])\\s*${sel.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s*\\{([^}]*)\\}`).exec(pCss);
    if (!m) return null;
  ...
  // --- 31.4 浮かせるボタンは2画面で同じ1部品 -------------------------------------
  {
    ...
    check("31.4 実装の高さは §5 のトークン(--tap-min)。縦横とも確保する",
      /minHeight: "var\(--tap-min\)", minWidth: "var\(--tap-min\)",/.test(fab));
    check("31.4 角丸はピル・地はアクセント・文字は面の上の文字色(いずれも既存トークン)",
      /borderRadius: "var\(--r-pill\)"/.test(fab)
      && /background: disabled \? "var\(--c-disabled\)" : "var\(--c-accent\)",/.test(fab)
      && /color: "var\(--c-on-accent\)",/.test(fab));
    check("31.4 影は既存の浮かぶ物と同じ値(新しい濃さを発明していない)",
    ...
    check("31.4 位置に px の直書きが無い(トークンだけで書かれている)",
      !/(right|bottom|top|left): "-?\d/.test(fab), (fab.match(/(right|bottom|top|left): "[^"]*"/g) || []).join(" / "));
    // スクロールで動かない + transform の外へ出す(track の中の fixed は画面基準にならない)
    check("31.4 position:fixed で、body へ portal する(SwipePager の transform の外へ出す)",
      /position: "fixed", zIndex: FLOAT_ACTION_Z,/.test(fab)
      && /return createPortal\(/.test(fab) && /document\.body,/.test(fab));
    check("31.4 重なりの順はシート(60)より下・下部ナビ(30)より上",
    ...
    check("31.4 どちらの呼び出しも「今このページか」を条件に持つ(隣のページで出したままにしない)",
      /pageActive=\{dataSubTab === "mydata"\}/.test(src)
      && /pageActive=\{reedsSubTab === "register"\}/.test(src)
      && (codeOf(src).match(/\{pageActive && \(/g) || []).length === 2);
  }
}

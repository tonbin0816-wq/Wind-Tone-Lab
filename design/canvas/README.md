...
足している画面があれば狭くなる。実測(375px 幅)はこうなる:

| どこ | 足す分 | 本文幅 | カード内の文字の左端 |
|---|---|---|---|
| My Data・分析・詳細2画面 | なし | 347px | 30px |
| リードタブ・リード個体詳細 | なし（B10 で +10 は消えた） | 347px | 30px（枠のカードは 31px） |
| コミュニティ | +16（`pageStyle` の `--sp-4`）※ | 315px | 46px |

> **リードタブの行は 2026-09-16 に直した。** 2026/09/09 の実測時は `REED_LIST_EXTRA_PAD_PX` で
> +10(本文 327px)だったが、同日の B10 裁定「案ア」で定数3つと包みの div 2枚が消え、`app-root` の
> 14px だけになった。Chrome 375×812 の実測(2026-09-16)でリード個体詳細のカードは x=14・幅 347・
> 枠 1px・padding 16 → 内側 313(`detail.mjs` の `REED_W`)。セッション詳細と同じ。
>
> ※ コミュニティの行は 2026/09/09 の実測のまま。`CommunityTab.jsx` の `pageStyle` はその後
> `padding: var(--sp-4) 0` になっている(+16 は撤去済み。`design/UNIFY-AUDIT.md` の B10)ので
> この行も 347px になっているはずだが、**実測して直すまで数字は書き換えない**
> (dev サーバは接続設定がなくコミュニティを描かないので、この周では測れなかった)。

**同じ 375px でも余白が違うのは実装がそうだから**で、モックの側で揃えてはいけない
(揃えるなら先に実装を揃える。B10 はそうして揃えた ── `design/UNIFY-AUDIT.md`)。

...
function sIdealCard(empty = false) {
  ...
  // 右端は ▾ ── 画面が変わるのではなく**選択肢のシートが開く**(自分の計測から / みんなのデータから)。
...
// ---- S1(採用) 閉じた状態: 日付を押していないので、セッションは出ていない --------
{
  ...
  writeFileSync(OUT + "S1.dc.html", dcFile(sPage(inner, `<b>採用案の既定の姿</b>(日付を押していない状態)。<br>本人指示で<b>その日のセッションは常時表示をやめた</b>ので、ここには出ていない ── カレンダーのすぐ下は「すべてのセッション」、その下が折れ線。<br><b>カレンダーの月見出し</b>は yyyy/m。合計(3.7 時間 · 16 日)は月見出しの<b>右・下揃え</b>(baseline)。文字の大きさが違っても足元が1本に揃い、<b>月の綴りが伸びても重ならない</b>(見た目のズラしを使っていないため)。<br>日付を押した状態は右の「S1 日付を押した状態」。<br><b>便G(2026-09-16 実機の指摘)</b>: 累計の時間は<b>「練習時間」</b>(音を感知していた時間の合計)。累計のカードは<b>押せる</b>(▾)── 押すと定義のシートが出る(練習時間 / 計測件数 / 計測音 の3行 + <b>集計対象「奏者が自分の計測データのみ」</b>の4行目。<b>説明文の左端は4行とも縦に揃う</b> ── ラベルの列を <code>flex: 0 0 4em</code> で固定。コミュニティのプロフィールの行と同じ作法)。主要動作は<b>「みんなのデータをみる」</b>(束1 2026-09-19 本人指示。行き先は順位の子タブのまま)。一番下に<b>「目安」</b>のカード(計測タブの詳細シートから移動)。<br><b>便I(2026-09-17 本人裁定)</b>: 目安が<b>0件でもカードは出す</b> ── この画面がその姿で、「目安を設定してください」と<b>導線1つ</b>(押すと「すべての計測」へ)。1件以上の姿は右の「S1 日付を押した状態」。<b>カレンダーの合計も同じ「練習時間」</b>(マスの濃さも)で、累計のカードと定義が揃った。`)));

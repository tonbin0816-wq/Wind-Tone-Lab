...
//
// 【この2画面の本文幅は同じ】どちらも 347px(375 − --page-side-pad 14 × 2)。カード内側 313px
//   ・セッション詳細 … 本文 347px。カード内側 313px(SESSION_W)
//   ・リード詳細     … 本文 347px。カード内側 313px(REED_W)
// 【R10 2026-09-16】カードが影から**枠**へ移ったので、`box-sizing: border-box` のぶん
// カードの内側が左右 1px ずつ狭まった(315 → 313)。
// **1.2px ではなく 1px** ── ブラウザは border-width を device px に丸めるので、
// 1.2px の宣言は dpr 1 でも 2 でも 1 CSS px で描かれる(Chrome 375×812 で実測: 347 の
// カードの内側が 313px)。
//
// 【以前ここには「リード詳細は本文 327px(左右 10px を足す)・カード内側 293px」と書いてあった】
// それは B10(2026/09/09 本人裁定「案ア」)より前の値。B10 でリードタブの左右余白の定数3つ
// (REED_SIDE_PAD_PX / REED_APP_SIDE_PAD_PX / REED_LIST_EXTRA_PAD_PX)と包みの div 2枚が消え、
// リード詳細も .app-root の 14px だけになった。**左右余白の唯一の答えは index.css の
// --page-side-pad**(App.jsx の B10 の注記と同じ)。
// 実測(Chrome 375×812、2026-09-16): リード個体詳細の .card.card-outline は
// x=14・幅 347・枠 1px・padding 16 → 内側 313。セッション詳細と同じ。
// 定数は画面ごとに2つのまま置く(実装が別々の画面なので、片方だけ変わったら片方だけ動かす)。
// 値が同じでも MetricTabCard の点の間隔や半径は幅から決まるので、幅を発明しないことが要。
//
// 数値と日付は**ダミー**。実データではない。
...
// ---- リード個体 ---------------------------------------------------------
const REED_W = 313; // 375 − 14×2 − 16×2 − 1×2(B10 以後はセッション詳細と同じ。上の注記。実測 2026-09-16)

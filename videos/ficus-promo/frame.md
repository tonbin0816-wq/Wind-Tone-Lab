...
description: >
  ...
  accent, the three-step gray text ladder, Instrument Serif (display/numerals/chrome) + Instrument Serif (body),
...
colors:
  ...
  text-muted: "#435266"
  text-light: "#8D95A1"
  accent-light: "rgba(23, 69, 133, 0.08)"
  accent-medium: "rgba(23, 69, 133, 0.15)"
  border: "rgba(23, 69, 133, 0.20)"
  card-bg: "rgba(23, 69, 133, 0.04)"
  positive: "#16A34A"
  negative: "#DC2626"
...
typography:
  # — reading ramp (Instrument Serif body + Instrument Serif chrome) —
  body:    { fontFamily: "-apple-system, BlinkMacSystemFont, \"Hiragino Sans\", \"Noto Sans JP\", sans-serif", cqw: 0.85, weight: 400, lineHeight: 1.6, color: "text-muted" }
  h4-eyebrow:{ fontFamily: "-apple-system, BlinkMacSystemFont, \"Hiragino Sans\", \"Noto Sans JP\", sans-serif", cqw: 0.8, weight: 400, tracking: "0.08em", upper: true, color: "primary" }
  tag:     { fontFamily: "-apple-system, BlinkMacSystemFont, \"Hiragino Sans\", \"Noto Sans JP\", sans-serif", px: 12, weight: 400, color: "primary" }
  counter: { fontFamily: "-apple-system, BlinkMacSystemFont, \"Hiragino Sans\", \"Noto Sans JP\", sans-serif", px: 13, weight: 400, tracking: "0.05em", color: "text-muted" }
  # — display / numerical ramp (Instrument Serif, near-black headings / cobalt numerals) —
  h3:      { fontFamily: "-apple-system, BlinkMacSystemFont, \"Hiragino Sans\", \"Noto Sans JP\", sans-serif", cqw: 1.25, weight: 400, lineHeight: 1.3, tracking: "-0.02em", color: "text" }
  stat-num:{ fontFamily: "Instrument Serif", cqw: 1.9, weight: 400, lineHeight: 1.0, color: "primary" }
  blockquote:{ fontFamily: "-apple-system, BlinkMacSystemFont, \"Hiragino Sans\", \"Noto Sans JP\", sans-serif", cqw: 2.4, weight: 400, lineHeight: 1.35, color: "text" }
  h2:      { fontFamily: "-apple-system, BlinkMacSystemFont, \"Hiragino Sans\", \"Noto Sans JP\", sans-serif", cqw: 2.6, weight: 400, lineHeight: 1.1, tracking: "-0.02em", color: "text" }
  metric-value:{ fontFamily: "Instrument Serif", cqw: 3.0, weight: 400, lineHeight: 1.0, color: "primary" }
  h1:      { fontFamily: "-apple-system, BlinkMacSystemFont, \"Hiragino Sans\", \"Noto Sans JP\", sans-serif", cqw: 4.2, weight: 400, lineHeight: 1.08, tracking: "-0.02em", color: "text" }
  quote-mark:{ fontFamily: "Instrument Serif", cqw: 8.0, weight: 400, lineHeight: 0.5, color: "primary", opacity: 0.15 }
...
components:
  ...
  cta-button:
    ...
    textColor: "{colors.bg}"
    rounded: "{radii.pill}"
    typography: "Instrument Serif 400"
...
- **Fonts** — the frontmatter is already correct and differs from the preset's remix in two places, ON PURPOSE:
  - **Body / headline / chrome = the Japanese system sans** (`-apple-system, Hiragino Sans, Noto Sans JP`).
    **Instrument Serif has no Japanese glyphs.** Every visible word in this video is Japanese, so
    setting copy in it would fall back to an arbitrary face. Never put Japanese text in Instrument Serif.
  - **Instrument Serif is for numerals and marks only** (`stat-num`, `metric-value`, `quote-mark`).
    That is the app's own rule (`design/DESIGN-SYSTEM.md` §4.3: the serif carries note names and reed
    numbers, nothing else). Keep it there.
- **Weights** — the brand font ships `{400}` only; every weight is clamped to these — ignore higher preset weights (e.g. 600/700) in prose.
- **Colors** — use the frontmatter hex; preset color NAMES in prose mean the remapped brand values:
  - **"cobalt" = `#174585`** — the app's `--c-accent`, the ONLY accent. (The automatic remix first
    picked the app's error red `#DC2626`; that was corrected by hand. **Nothing in this video is red**
    except a genuine error state, and there are none.)
  - **"cream" / "warm cream ground" = `#FFFFFF`** — the app's ground is **pure white**, not cream.
    The app moved every screen to a white ground on 2026/09/06. Do not paint a cream canvas.
  - The gray ladder is the app's ink ladder: `#121F32` → `#435266` → `#8D95A1`.
...
The voice is two faces in fixed roles: **Instrument Serif** (display, every numeral, all chrome —
eyebrows uppercase 0.08em) and **Instrument Serif** (body, muted gray, line 1.6). Headlines are near-black;
...
- **Instrument Serif** (display/numerals/chrome) + **Instrument Serif** (body) — near-black headlines, cobalt numerals.
...
Two ramps. The **reading ramp** (Instrument Serif body 0.85cqw muted; Instrument Serif eyebrow uppercase 0.08em
cobalt) carries copy + chrome; the **display/numerical ramp** (Instrument Serif `h3` 1.25cqw → `h1`
...
- **Headlines near-black, −0.02em**; **eyebrows cobalt, uppercase, 0.08em**; **numerals cobalt 600–700**; **body Instrument Serif 400 muted, line 1.6**. No italic, no uppercase body, no cobalt headline.
...
**Composes** accent-line, meta, h1, body sub. **Focal** a 2-line Instrument Serif `h1` near-black, left,
...
**Focal** a row of tinted cards — cobalt `metric-value` + Instrument Serif label + muted desc + optional
...
blockquote, cite. **Focal** a Instrument Serif `blockquote` near-black under a 15%-opacity cobalt
...
Instrument Serif body column beside a cobalt-8% highlight block (4px cobalt left rule) carrying an inline pull
...
a Instrument Serif `h1` near-black, centered, with the one solid cobalt `cta-button` pill below. **Accent**
...
- Use **tinted cards** (4% fill, 20% border, 10–14px radius, no shadow); body Instrument Serif 400 muted, line 1.6.
...
- **Type** — Instrument Serif headings −0.02em near-black, cobalt eyebrows 0.08em + numerals; Instrument Serif body muted line 1.6; ≥1.4cqw floor.
...
- **Instrument Serif + Instrument Serif via Google Fonts.** CJK pairing (Noto Sans SC 700 display / Noto Serif SC 400 body) carries over; the eyebrow's uppercase+tracking signal weakens in CJK — pair it with the accent-line.

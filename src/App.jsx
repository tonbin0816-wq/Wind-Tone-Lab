...
// 音量(dB)。画面に出るのは MeasureView の環の下の `volumeDb.toFixed(1)` **1箇所だけ**(M9/M10)。
...
// ============================================================
// リード登録用マスタデータ
// メーカー・銘柄: 正は src/community/catalog/gear.js の REED_CATALOG(ここでは持たない)
//             + ユーザーが自由入力したメーカーを自動追加
// 番手: 2.0〜4.0を0.25刻み(正は src/community/profile.js。ここでは持たない)
// ============================================================
// 【R6 2026-09-16 本人裁定③】以前ここには INITIAL_REED_BRANDS(7メーカー)を直書きしていた。
// コミュニティのカタログの部分集合で、片方だけ増える形になっていたので**カタログ1つに畳んだ**
// (13メーカー + 銘柄)。カタログに無いメーカーの逃げ道(REED_BRAND_CUSTOM の自由入力)は残してある。
const REED_BRAND_OPTIONS = Object.keys(REED_CATALOG);
// そのメーカーの銘柄の選択肢。自由入力のメーカーと、カタログに無い古い記録のメーカー
// ("Rico (D'Addario)" 等)は空配列 ── 呼び出し側は銘柄の行ごと出さない。
function reedModelOptions(brand) {
  return REED_CATALOG[brand]?.models ?? [];
}
// 保存する銘柄。**カタログにある組だけを通す**(自由入力のメーカーに銘柄は付かない)。
...
// 決められない。メーカーが1つに収まるならその名で呼び、またがるときは数だけで言う。
// 箱の呼び名は一覧の見出しと同じ綴り(`${brand} ${strength}`)にする。
//   ・箱を消した: 「Vandoren V16 3.0 の2箱(6枚)」 / メーカーが複数なら「2箱(6枚)」
//   ・枚を消した: 「Vandoren V16 3.0 の3枚」     / メーカーが複数なら「3枚」
function deletedReedsLabel(groups, count, boxCount) {
  // 【R6 2026-09-16】銘柄を持つ箱は銘柄まで言う(一覧の見出しと同じ綴り = reedBrandModelLabel)。
...
//
// 短縮は**メーカーの最後の語**(=銘柄。"Vandoren V16" の "V16"、"D'Addario Select" の "Select")を採る。
// ただし登録済みのメーカーの中で最後の語が衝突する場合(例: "Vandoren Java" と "Marca Java")は、
// **そのメーカーだけ**フルのメーカー名に戻す。衝突しているのに同じ表記になると、どの箱を選んでいるのか
// 画面から判別できなくなるため。判定は「いま登録されているメーカーの集合」に対して行うので、
// メーカーが増えて衝突が生まれた時点で自動的にフル表記へ切り替わる。
// 番手は末尾の ".0" だけ落とす("3.0"→"3" / "2.5"→"2.5")。
// 【M5 2026-09-16 実機の指摘】計測タブの上部設定行は 箱(メーカー)と厚さを**別の語**として
// 別の色で描くので、短縮の規則を2つに割って取り出せるようにした。
// **規則そのものは1つのまま**: shortBoxLabel は下の2つを `-` で繋いだものと定義する
// (メーカーの短縮と番手の `.0` 落としを2箇所に書かない)。返す文字列は1文字も変わっていない。
function shortBrandLabel(brand, allBrands) {
  ...
  return collides ? b : last;
}
function reedStrengthLabel(strength) {
  return String(strength ?? "").replace(/\.0$/, "");
}
// 【R6 2026-09-16 本人裁定③】箱の短い呼び名の**頭**。
// 銘柄(model)を持つ箱は銘柄で呼ぶ ── 銘柄こそが箱を言い当てる語で、
// メーカーの最後の語を採る短縮(shortBrandLabel)は銘柄が無い時代の代用だった。
// 銘柄を持たない既存の記録は従来どおりメーカーの短縮に落ちる(1文字も変わらない)。
...
// 【R6】長いほうの呼び名「Vandoren V16」。銘柄が無ければメーカーだけ(既存の記録と同じ)。
...
// リードでは表示だけの問題では済まない:
//   - reedGroupKey は `メーカー|番手|開封日` なので、同じ実日の 08:00 と 10:00 で**箱が2つに割れる**
...
const MIC_RECOVER_FAILED_MSG = "マイクを再接続できませんでした";
    ...
    <div className="app-root" style={{ background: "var(--c-bg)", color: "var(--c-ink)", fontFamily: "var(--font-jp)", padding: "calc(16px + env(safe-area-inset-top)) var(--page-pad-right) var(--page-bottom-gap) var(--page-pad-left)", boxSizing: "border-box" }}>
      ...
      {/* 【面の作法】作法のクラスは DESIGN-SYSTEM §6.6 / index.css が決める。
          ...
          **根に作法のクラスを1つ付けるのは計測タブだけ**になった(他は各 return が名乗る)。
          ...
          共有部品(PhraseTimeline・MetricTabCard)を分岐なしで置ける。 */}

      {/* リードタブ: 子タブ(登録 / 比較) + 本体。
          【N-5】子タブの行は ReedsTab の中へ移した。正典 .subtabs は「素のテキスト2つ」。
          溝つきのセグメントコントロールは正典に無いので撤去した。
          【便R 後半-後半 2026-09-21】右端の「…」と、その中の削除モード2つは**定義ごと無い**
          (本人「個体一枚ずつ消すことはほぼないのでなくていい」)。箱の削除と番号編集は
          箱の編集シートの下の2つが担う。開封日の編集も同じシート。
          **データタブの子タブ行(AnalysisLabView 側)には触っていない。** */}
      {topTab === "reeds" && (
      ...
      {errorMsg && (topTab === "measure" || (topTab === "analysis" && !ERROR_MEASURE_ONLY.includes(errorMsg))) && (
        ...
        >
          <div style={{ width: "100%", maxWidth: 900, background: "var(--c-surface)", borderRadius: "var(--r-lg)", padding: "var(--sp-4)", boxShadow: "0 8px 24px rgba(15,23,42,0.18)" }}>
            ...
            {ERROR_MEASURE_ONLY.includes(errorMsg) && (
              <>
...
// 計測タブの上部設定行の縦の寸法。**上部設定行の高さ 60px を保ち、環を動かさない**
// (§6.1.5)ためだけに在る2つ。
// 下の2つの値は**新しく決めた寸法ではなく、Chrome 375×812 で実測した現行の値**:
//   奏者の枠 = 28px / リード枠の中の値を包む箱 = 26px
// 【便D で中身が変わった】以前はここに透明なネイティブ <select> を重ねており、
// 「appearance を落としたら縦も自分で決める」(§6.7)ための寸法だった。
// M4 でネイティブのフォームコントロールが1つも無くなったので、いまは
// **素の <button> / <span> の高さ**そのものになっている(役目は同じ)。
//
// 【ここは Chrome では判定不能・実機待ち(iOS Safari)】
// 上の2つは「どの端末でも同じ高さになる」ことの証明にはならない。箱の高さは数値で
// 決めてあるが、**その中に載る文字の行送りと字形は書体とエンジンで変わる**ので、
// 26/28px の箱に 12px の文字が Chrome と同じ位置で収まるかどうかは、
...
// その上下に 2px ずつの padding を持っていたので、**行の高さは 30** だった。
// M4 で枠が <button> 1枚になり、padding は border-box の中に入る(index.css の `*` 規則)。
// そこでこの定数は**枠そのものの高さ**を持つ = 26 + 2 + 2 = 30。
// 中身の高さ(26)も、行の高さ(30)も、上部設定行の高さ(60)も**実測で不変**
// (Chrome 375x812: 1行目 28 + gap 2 + 2行目 30 = 60)。
// ここを 26 に戻すと行が 4px 縮み、§6.1.5 の「環を動かさない」が破れる。
const TOPSET_REED_SELECT_H_PX = 30;
// 【M1 2026-09-16 実機の指摘】上部設定行の**左端の余白**。本人指摘「1行目の奏者名の左端が
// 2行目の青い点の左端と揃っていない」。1行目(奏者)と2行目(リード枠)が**この1つの定数**を
// 読むことで、片方だけずれる事故を構造ごと無くす。値は2行目の枠がもともと持っていた
// 左 padding(10px)そのもので、新しく決めた寸法ではない。
const TOPSET_ROW_PAD_LEFT_PX = 10;
...
//
// 【2026/09/10 本人指示で半拍ずらした ── cos → sin】
// 本人「振り子のアニメーションと音が合っていない。少しだが音の方が先行している。
// 振り子が真ん中に来るタイミングで音が鳴るようにして」。
//
// 【何が起きていたか】音の予約時刻 t がそのまま振り子の錨(metroAnchorRef)なので、
// **音が鳴る瞬間の位相はちょうど整数**になる。cos(π*整数) は ±1、つまり
// **音は振り子が端に居るときに鳴っていた。**
// 端は振り子がいちばん遅くなって留まる所で、目が「拍だ」と読むのは
// **いちばん速く通り過ぎる中央**のほう。だから音が先に来たように感じる。
//
// sin(π*phase) にすると、位相が整数のとき 0 = 中央。往復の周期(片道1拍)は変わらない
// (cos と sin は角振動数が同じ)ので、停止後の戻り(F-51)が引く ω = π/beatDur もそのまま。
//
// 【端の膨らみも一緒に中央へ動く】ringBeatEmphasis は「拍の瞬間に最大」で、
// 拍の瞬間 = 音の瞬間。2026/08/16 の本人指示「振り子が端に触れるときの大きくなるのも
// 今よりも大きくして」は、当時**端 = 拍**だったからそう書かれている。
// 膨らみが marking しているのは端ではなく拍なので、拍が中央へ来れば膨らみも中央へ来る。
// **倍率は1つも変えていない。**
//
// 停止中(phase===null)は静止した振り子と同じく最下点=12時(0°)に置く。
// sin では拍の瞬間も 0 なので、**動かし始めに端へ飛ばなくなった**(cos のときは
// 止まっている 0° から、最初の拍でいきなり端 ±SWING へ跳んでいた)。
function ringPendDeg(phase) {
  if (phase === null || phase === undefined) return 0;
  return RING_PEND_SWING_DEG * Math.sin(Math.PI * phase);
}
...
function MeasureView(props) {
  ...
  // 【便D(M9)で音色の表示が消えた。ここは触っていない】倍音構成・重心・HNR・目安の
  // 表示は M9 で詳細シートごと無くなったので、この3つ(と matchedFingering)には
  // **いま画面の読み手が無い**。それでもゲートと配線を残してあるのは、
  // D-19/D-20/D-21 の「表示値づくりの費用を測る計器」がこの経路そのものを観測対象に
  // しているため(検証37〜39)。計器ごと畳むのは便D の担当範囲の外なので、
  // **次便へ回す**(このコメントが起票の代わり)。勝手に消すと診断の数字が意味を失う。
  ...
  const [openPicker, setOpenPicker] = useState(null); // null | "tuning" | "sax" | "performer" | "box" | "reed"
  const [detailOpen, setDetailOpen] = useState(false); // 詳細カード(音量表示 / 計測下限dB)の開閉。デフォルトは閉じておく
  // 【M9/M10 2026-09-16】計測タブで音量(dB)を出すかどうか。詳細シートのスイッチが切り替える。
  // 既定は OFF ── いままで環の下に dB は出ていなかったので、**設定を触るまで見た目は変わらない**。
  const [showVolume, setShowVolume] = usePersistedState("showVolume", false);
  ...
  const dViewTree = (
    <div ref={measureRootRef} style={{ maxWidth: 900, margin: "0 auto" }}>
      ...
      <div className="tap-through" style={{ position: "relative", zIndex: 1, opacity: isRecording ? 0.35 : 1, display: "flex", alignItems: "center", gap: 8, marginBottom: "var(--sp-1)" }}>
        ...
        {/* 【M1 2026-09-16】左端は2行目と**同じ定数**(TOPSET_ROW_PAD_LEFT_PX)。
            この行の先頭は奏者なので、行に敷けば奏者名の左端が2行目の青い点と揃う。 */}
        <div className="sans" style={{ fontSize: 12, display: "flex", alignItems: "center", gap: 6, flexWrap: "nowrap", overflowX: "auto", paddingLeft: TOPSET_ROW_PAD_LEFT_PX }}>
          {/* 1行目 = 奏者 / 楽器種別 / 基準ピッチ。**表示と選択の両方**を担う(本人指示)。 */}
          {/* 【M4 2026-09-16】奏者もアプリのピッカーで選ぶ。開閉の状態は**この画面が持つ**
              (openPicker が null でない間は背面レイヤと録音ボタンを disabled にするため)。 */}
          <PerformerSelector
            performers={performers} selectedPerformer={selectedPerformer}
            setSelectedPerformer={setSelectedPerformer} setPerformers={setPerformers}
            disabled={isRecording}
            selectId="measure-performer-select"
            pickerOpen={openPicker === "performer"}
            onOpenPicker={() => setOpenPicker("performer")}
        ...
        <div className="sans" style={{ fontSize: 12, display: "flex", alignItems: "center", gap: 6, flexWrap: "nowrap", overflowX: "auto" }}>
          {/* リード選択の枠。
              ...
              **地も枠も持たない素のテキスト + 山形** なので、B型 .ctl-plain(地 --c-sunken)を外した。
              選択済み/未選択は左の点の色と文字の色・太さが返す(地では返さない)。
              【D-2】色は正典に合わせて 箱=--c-ink(太字) / 個体=--c-ink-2。
              以前は箱も個体も --c-accent で、DESIGN-SYSTEM §1.4「--c-accent はアクション専用」に
              反していた(この枠は押せるが、文字色が状態を主張する必要は無い)。
              【便D で当たり判定の作りが変わった】以前は「枠まるごとを箱の <select> の
          ...
          {/* 【M3/M4/M5/M6 2026-09-16 実機の指摘】native <select> をやめ、楽器と
              ...
              ・M4 押した結果**必ずアプリのピッカーが開く**。<label> の activation behavior が
                プラットフォーム依存だった問題(F-72 の未解決点)は構造ごと消えた
              ・M5 並びは [点] 箱(メーカー) 厚さ 日付 個体# と末尾の山形。メーカーと厚さは**別の語**に
                分けて色で段を付ける(箱=--c-ink 太字 / 厚さ=--c-ink-2 / 日付=--c-ink-3 /
                個体#=--c-ink-2)。区切りは記号ではなく余白 --sp-1
              ・M6 日付の欄は**1つ**(箱の開封日)。個体まで選んでも同じ値。中黒は廃止
          ...
          <div style={{ display: "flex", alignItems: "center", flexShrink: 0 }}>
            ...
            <button
              ...
              aria-label="リードの個体を選ぶ"
              aria-haspopup="listbox"
              style={{ pointerEvents: "auto", display: "flex", alignItems: "center", height: TOPSET_REED_SELECT_H_PX, padding: "2px 4px 2px var(--sp-1)", background: "none", border: "none", font: "inherit", color: "inherit", flexShrink: 0, cursor: isRecording || !selectedBoxGroup ? "default" : "pointer" }}
            >
              <span style={{ color: selectedReedId ? "var(--c-ink-2)" : "var(--c-line-strong)", whiteSpace: "nowrap", maxWidth: 60, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis" }}>
                {(reedMemberOptions.find((o) => o.value === (selectedReedId || "")) || reedMemberOptions[0]).label}
              </span>
              {/* 正典 .reedchip の末尾の山形。枠(button)の中なので当たり判定の穴にならない。 */}
              <PickChevron />
            </button>
          </div>
        </div>
        ...
        <button
          ...
          style={{
            /* 【M7 2026-09-16 実機の指摘】「メトロノームの当たり判定を上だけ下げる」。
               ...
               このボタンが受け取っていた。高さを §5 の最小(--tap-min)にし、
               alignSelf: flex-end で**下端に寄せる**ので、上端がそのぶん下がる。
               行の高さ(60px)は左の列が決めているので、環は 1px も動かない(§6.1.5)。 */
            display: "inline-flex", alignItems: "center", justifyContent: "center",
            width: "var(--tap-min)", height: "var(--tap-min)", alignSelf: "flex-end",
      ...
      <div style={{ flexShrink: 0 }}>
        ...
        {/* 【M9/M10 2026-09-16】音量(dB)。詳細シートの「音量表示」が ON のときだけ出す。
            **環より下**なので、切り替えても環・上部設定行は 1px も動かない(§6.1.5)。
            ...
            表記の段は録音の経過時間と同じ(--font-num / 12px / --c-ink-3)で、新しい値を足さない。 */}
        {showVolume && (
          <div className="sans" style={{ marginTop: "var(--sp-1)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-num)", fontSize: 12, color: "var(--c-ink-3)" }}>
      ...
      {/* 詳細トグル: 【M9 2026-09-16】中身は「音量表示」と「計測下限dB」の1行だけになった。
          画面の一番下(録音ボタンより下)に置く。 */}
      ...
      {/* ── 追加表示 ── 詳細カード。固定枠の「外・下」に置く。開いても枠の高さは measureMinH の
          ...
          覆われない。中の操作要素(音量表示のスイッチ・計測下限スライダー)は
          そのまま自分でタップを受け取る。 */}
      {detailOpen && (
          ...
          <div className="card no-top-rule">
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "var(--sp-3)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "var(--sp-1)", flexShrink: 0 }}>
                <span className="sans" style={{ fontSize: 12, color: "var(--c-ink)", fontWeight: 700 }}>音量表示</span>
                {/* スイッチ = 「触った瞬間に効く設定」(チェックボックスは「保存で効く申告」)。
                    作法はコミュニティの「公開」と同じ: role="switch" + aria-checked、
                    当たり判定は軌道ではなく 44px 角のボタン、軌道 44x26・つまみ 20。
                    **部品は import しない**: あれは遅延読み込みのコミュニティ側にあり、
                    ここから import すると計測タブの起動にコミュニティ一式が乗る。 */}
                <button
                  type="button" role="switch" aria-checked={showVolume} aria-label="音量表示"
                  onClick={() => setShowVolume((v) => !v)}
                  className="no-select"
                  style={{ flex: "0 0 auto", width: "var(--tap-min)", minHeight: "var(--tap-min)", padding: 0, background: "transparent", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
                >
                  <span style={{ display: "block", width: 44, height: 26, borderRadius: "var(--r-pill)", position: "relative", background: showVolume ? "var(--c-accent)" : "var(--c-line-strong)" }}>
                    <span style={{ position: "absolute", top: 3, left: showVolume ? 21 : 3, width: 20, height: 20, borderRadius: "50%", background: "#fff", boxShadow: "0 1px 2px rgba(0,0,0,.2)" }} />
                  </span>
                </button>
              </div>
              {/* 計測下限dB: バンドパス後の音量がこの値以下なら無音とみなす(旧称ノイズゲート)。 */}
              <div style={{ display: "flex", alignItems: "center", gap: "var(--sp-1)", flex: 1, minWidth: 0 }}>
                <span className="sans" style={{ fontSize: 12, color: "var(--c-ink)", fontWeight: 700, flexShrink: 0 }}>計測下限dB</span>
                <input
                  type="range" min="-80" max="-20" step="1" value={noiseGateDb}
                  aria-label="計測下限dB"
                  onChange={(e) => setNoiseGateDb(Number(e.target.value))}
                  style={{ flex: 1, minWidth: 0, accentColor: "var(--c-accent)" }}
                />
                <span style={{ fontFamily: "var(--font-num)", fontSize: 13, fontWeight: 700, color: "var(--c-accent)", width: 62, textAlign: "right", flexShrink: 0 }}>{noiseGateDb}dB</span>
              </div>
            </div>

            {/* 端末がAGC等を無効化できなかった場合の警告(iOS Safari等で発生しうる)。
                **指標ではなく過渡的な告知**なので M9 の削除の対象ではない(出るのは失敗時だけ)。 */}
...
// 本人の実機指摘として挙がったため。**選択肢の綴りは呼び出し側が options で渡す**
// ({ value, label } の配列。見えている値の text と同じ出どころにすること)。
...
// 【D-5 2026/08/23 本人指示】caption: 小さいラベルを**同じ <label> の中**に入れる任意指定。
// 本人の実機指摘「並べる軸、数値、分け方の縦幅が無駄に大きい」への答え —
...
function PlainSelect({ text, value, onChange, options, ariaLabel, strong = false, caption = null }) {
  ...
  return (
    <>
      <button
        ...
        style={{ display: caption ? "flex" : "inline-flex", flexDirection: caption ? "column" : "row", alignItems: caption ? "flex-start" : "center", justifyContent: "center", minHeight: "var(--tap-min)", minWidth: "var(--tap-min)", background: "none", border: "none", padding: 0, font: "inherit", color: "inherit", cursor: "pointer", maxWidth: "100%" }}
      >
...
// 【D-5 2026/08/23 本人指示・凍結仕様 design/D5-SPEC.md】タイムラインは
// **表示=ピッチ / 基準=絶対値に固定**。「それぞれ選択できる機能を削除」(本人)。
// 正典 #14b も同じ("「表示：ピッチ」「基準：絶対値」の選択UIはタブと重複するので廃止")。
...
function PhraseTimeline({ frames, noteEvents, barlines }) {
  ...
  return (
    <>
      {/* 【D-5 2026/08/23 本人指示】ここにあった「表示: ピッチ ▾ / 基準: 絶対値 ▾ / 別セッション ▾」の
          行は**削除した**(「表示をピッチ、基準を絶対値に固定。それぞれ選択できる機能を削除」)。
      ...
      <div style={{ padding: "10px 0", marginBottom: 10 }}>
        <div className="sans" style={{ fontSize: 12, color: "var(--c-ink-2)", marginBottom: 8 }}>
          ...
          タイムライン
          {noteEvents?.length > 0 && (() => {
            // 【D-5 2026/08/23 実機確認で発見】!== null では **undefined が通り抜ける**。
            // アタック時間を持たないノートが1つでも混ざると合計が NaN になり、
...
// 箱(同じメーカー・番手・使用開始日の群)の平均総評。**総評そのものと同じ 0.1 刻みに丸める**(F-62)。
...
// <select> を重ねる」形だった(F-72 / 差し戻し①)。これを捨てた理由は2つ:
...
// 録音ボタンを disabled にする」(§6.1.5)ので、開いているかどうかを画面側が知る必要がある。
...
function PerformerSelector({ performers, selectedPerformer, setSelectedPerformer, setPerformers, disabled, selectId, pickerOpen = false, onOpenPicker, onClosePicker }) {
  ...
  return (
    <>
      {/* 正典 design/north-star-measure.html の .set1 は「自分 ▾」= 地も枠も持たない
          素のテキスト + ▾。font: inherit で行の書体・大きさをそのまま継ぐので、
          <label> + <span> だった頃と見た目は変わらない。
          【幅の上限は持たせない】枠の中に続くのが ▾ だけなので、上限が無ければ箱の幅は
          値そのものになる。上限を付ける = px を決めることなので発明しない。
          歯止め(overflow / textOverflow)は上限を持ったときに効くよう入れてある。 */}
      <button
        id={selectId}
        type="button"
        ...
        style={{ pointerEvents: "auto", display: "inline-flex", alignItems: "center", height: TOPSET_PERFORMER_H_PX, flexShrink: 0, background: "none", border: "none", padding: 0, font: "inherit", color: "inherit", cursor: disabled ? "default" : "pointer" }}
      >
...
// 【便AF 2026-09-21 本人指示】「リードタブの編集中の鉛筆マークを削除して、揺れてる時に
// カードタップで番号編集の仕様に変更」。カードの右上に載せていた印は**綴りごと消えた**
// (定数・目印・読み上げの名・lucide の読み手まで)。番号編集への入口は
// 「揺れている間(編集中)のカードのタップ」ただ1つ ── 下の ReedRegisterView の
// onTileTap が editing で行き先を分ける。**寸法や色の控えもここには残さない**
// (残すと次に同じ物を足す者への設計図になる)。
//
// 【AD-1 2026-09-21 本人指示】マス(タイルの包み)であることの目印。**綴りは1箇所**
// (下の reedListPressEndsEditing が同じ定数から選択子を組む)。カード本体はこの包みの
// 中に居るので、「タイルのどこかを押したか」はこの1つで見分けられる。
...
//   ・タイル(マスの中 = カード本体・マスの余白) … 終えない。
//     押した先の仕事(揺れていないときのタップ=個体詳細 / 揺れている間=番号のシート)は今までどおりで、並び替えも壊さない。
...
// 押下は包みまで上がってくる。ここで「押したことを覚えるか」だけを決める
// (stopPropagation は使わない ── カードのどこを掴んでも並び替えが始まる形を壊さないため)。
// **1枚しかない箱でも押下は記録する**(記録しないとタップが死んで詳細を開けない。N-5 の実測)。
// 【便AF 2026-09-21 本人指示】「リードタブの編集中の鉛筆マークを削除して、
// 揺れてる時にカードタップで番号編集の仕様に変更」。
// **鉛筆は綴りごと無くなった**ので、押下から除く相手も無い。
// マスの中で押されたものは、押下を記録し、並べ替えられるなら長押しも張る ── それだけ。
// (何を開くかは onTileTap が編集中かどうかで決める。ここは「押したことを覚えるか」だけ。)
...
function ReedTileGrid({ members, reeds, sessions, selectedReedId, editing, onEnterEditing, onTileTap, onReorder }) {
  ...
  // 【AA-2 / 便AF 2026-09-21】番号編集モードは入口ごと消えた(入口は揺れている間のカードのタップ1つ)ので、
  // 判定に残るのは「並び替える先があるか」だけになった。
  const canReorder = members.length >= 2;

  const handlePointerDown = (id, index) => (e) => {
    // 押下を覚えるかどうかを**いちばん先に**決める。
    // (便AF で鉛筆が消えて除く相手が無くなったので、いまは常に覚える。
    //  それでも先に置いたままにするのは、除く相手がまた増えたときに
    //  「落下の打ち切りだけが先に走る」穴を作らないため。)
  ...
  return (
    ...
    >
      {orderedMembers.map((r, home) => {
        ...
        return (
          /* 【便AF 2026-09-21 本人指示】マスの中身は**包み / カード**の2つ
             (鉛筆は便AF で消えたので、押せるものはカード1つだけ)。
             ...
             ・カード(<button>)… 見た目(.reedtile)と、編集中の揺れ。
...
// (【M9 2026-09-16】MetricCard = 音量 / スペクトル重心 / HNR の3枚を描いていた部品は
//  ここにあったが、本人の実機指摘「詳細シートの指標を音量表示と計測下限dB以外削除」で
//  **呼び手が0件になった**ので部品ごと消した。使い手の無い部品を残さない。)
...
// 番号を変える入口は**揺れている間のカードのタップ**で、「モードへ入ってからタップする」2段が
...
// 1枚ぶんの番号編集シート。**揺れている間にカードを押すと直接開く**
...
// ============================================================
// リード登録タブ (企画書10.2/10.3節) — メーカー/番手プルダウン化、10枚まとめ登録に対応
// ============================================================
...
function reedDetailMetaParts(startDate, days, sessionCount) {
  return [
    ...
    // ための記号で、分かっている 0 に使うのは意味が違う(ラベルの無い行では何が無いかも言えない)。
    `計測${sessionCount}件`,
  ].filter(Boolean);
}
...
// **並びは画面に出ていた順のまま**(厚さは上が 4.0、枚数は上が 10)── 選び方の部品が
// 変わっただけで、どの値がどの向きに並ぶかは本人の指示に入っていない。
const REED_ADD_COUNTS = Array.from(
  { length: REED_ADD_COUNT_MAX - REED_ADD_COUNT_MIN + 1 },
  (_, i) => REED_ADD_COUNT_MAX - i);
// 【`[...X].reverse()` と書かない】検査ハーネスの extractConst は `= [` で始まる定義を
// 角括弧の対応で切り出すので、閉じ括弧の**後ろに続く `.reverse()` を落として**しまう
// (落ちたまま「並びは正しい」と通ってしまう = 構造上失敗し得ない検査になる)。
// 先頭を識別子にして「`;` まで」の経路へ乗せる。**値は1つも変わっていない**。
const REED_STRENGTH_OPTIONS = REED_STRENGTHS.slice().reverse();

// 【便N 2026-09-19 本人指示】箱のシートの行の**形は1つ**。
// 本人の言葉:「メーカー選択 / 銘柄選択 / 厚さ選択に変更」「表示はメーカー選択などと同じ左寄せ」。
// 名札 → 値(左寄せ) → 右端に ▾ の3つで、高さは §5 の当たり判定 --tap-min。
// **新しい値を発明していない**: 名札の体裁(12px / --c-ink-3 / flexShrink:0)と間隔(gap 12)は
...
function ReedBoxSheet({
  ...
  return (
      <BottomSheet ariaLabel={isEdit ? "箱を編集" : "リードを追加"} onClose={onClose}>
          ...
          {/* 【便N】枠(名札 / 高さ --tap-min / 下の罫)は上の3行と同じ REED_SHEET_ROW_STYLE。
              上下の padding を足さないのは、中の input が自分で --tap-min の高さを持つため
              (足すと この行だけ 60px になって「4行が同じ高さ」が崩れる)。 */}
          {/* 【便N】height も併記するのは**実測で 45px になったから**(Chrome 375×812)。
            ...
            <div style={{ ...REED_SHEET_ROW_STYLE, height: "var(--tap-min)", borderBottom: "none" }}>
              ...
              <input
                ...
                /* appearance / maxWidth / lineHeight / overflow の4点は、N-5 まで
                   ...
                   【便N 2026-09-19 本人指示「開封日の日付も同じく左寄せ」】textAlign:left と
          ...
          {isEdit ? (
            <div style={{ display: "flex", gap: "var(--sp-2)", marginTop: "var(--sp-4)" }}>
              {/* 【AA-2 2026-09-21 本人指示】ここに在った「番号編集」は**綴りごと消えた**。
                  番号を変える入口は揺れている間のカードのタップ1つになり、モード(listMode)そのものが
...
function ReedRegisterView(props) {
  ...
  const openBoxEdit = (g) => {
    ...
    // 【R6】銘柄を持たない古い箱は null のまま開く(勝手に先頭の銘柄を入れない)。
  ...
  const registerReeds = (count) => {
    ...
    const newReeds = Array.from({ length: count }).map((_, i) => ({
      id: generateId(),
      brand,
      // 【R6 2026-09-16 本人裁定③】銘柄。カタログにある組だけを通す
      // (自由入力のメーカーは null)。既存のリードは model を持たないままでよい。
    ...
    >
      {reeds.length === 0 ? (
        ...
        reedGroups.map((g) => {
          ...
          // 「モード中は素の <span> に戻す」枝は読み手ごと消した(番号編集は揺れている間のカードのタップが直接開くので、
          ...
          return (
            ...
            <div key={g.key} style={{ paddingTop: REED_GROUP_PAD_TOP_PX, paddingBottom: REED_GROUP_PAD_BOTTOM_PX, borderBottom: "1px solid var(--c-line)" }}>
              ...
              <ReedTileGrid
                ...
                /* 【便AF 2026-09-21 本人指示】「揺れてる時にカードタップで番号編集」。
                   揺れている間(編集中)はカードを押すと**番号編集**、
                   揺れていないときは今までどおり個体詳細。
                   **押す物は1つで、行き先が状態で入れ替わる。**
                   (便AA〜AC は右上の印に番号編集を持たせていたが、本人の
                    「鉛筆がありすぎて集合体恐怖症みたい」でその印ごと無くなった。
                    番号編集への入口はここ1つだけ。) */
      ...
      {/* 【AA-2 2026-09-21 本人指示】番号編集モード中の案内「番号を変更するリードをタップ」は
          **綴りごと消えた** ── モードが無くなり、番号のシートは揺れている間のカードのタップが直接開くので、
          「どれをタップするか」を言う相手が居ない。
...
//  ここにあったが、本人指示「他のグラフと同様に表示グラフは一つでタブ切り替えで見れるように」で
//  グラフが1枚になり、並びは **DETAIL_CARD_METRICS**(セッション詳細・リード詳細と同じ)へ寄せた。
//  これで4画面すべてが同じ4指標・同じ並びのタブになる。読み手が無くなったので定義ごと削除。)
    ...
    <div>
      ...
      {items.length === 0 ? (
        ...
        <div style={{ paddingTop: 18 }}>
          ...
          {/* 【D-5 2026/08/23 本人指示】「他のグラフと同様に表示グラフは一つでタブ切り替えで
...
function buildNoteMatrix(valueByIdx, targetByIdx, count, saxType, tuningHz) {
  ...
  // 【D-5 2026/08/23 本人指示】**上の行から高い音**(「上が高い音の方が自然」)。
...
// **D-1 の発散カラースケール(8段の色)は廃止した。** 本人の実機指摘:
// 「色だとどっちの色が高い低いか想像しづらく、グラデーションもどれくらい高いかの程度が読みにくい」。
// 本人の提案「窓の縦方向の中央を0として、高ければその分中央から上方向に窓の中で棒を伸ばす」を採る。
// 見た目の出どころは design/mydata-cell-proposals.html の**案B1**。
...
// 【D-5 2026/08/23 本人指示・凍結仕様 design/D5-SPEC.md】練習カレンダーは**月表示**へ。
// 本人の実機指摘「カレンダーが一目でカレンダーとわからない。月と日付も入れてほしい」。
// D-1 の「今日を含む直近5週(35日)」は**役目を終えた**(見出しも「この5週の練習」から
// 「2026年8月」へ変わり、前後の月へ移動できる)。
// 曜日は**日曜始まり**(参考画像の一般的な暦に合わせる。5週窓の月曜始まりから変更)。
// 暦日は localDayKey ただ1つで組み立てる(罠14: toISOString の UTC 暦日は JST 00〜09時で前日にずれる)。
const CALENDAR_WEEK_LABELS = ["日", "月", "火", "水", "木", "金", "土"];
// その月の1日から末日までのマス。月初の曜日ぶんだけ**先頭に null**(空きマス)を置く。
// 戻り値の要素: null または { key, date, day, minutes, count }
function calendarMonthDays(sessions, year, month) {
  const byDay = {};
  for (const s of sessions || []) {
  ...
  const lead = first.getDay();                       // 日=0 … 土=6
  ...
  for (let d = 1; d <= lastDay; d++) {
    ...
    cells.push({ key: k, date, day: d, minutes: hit.minutes, count: hit.count });
  }
  return cells;
}
// その月の合計(見出しの右に出す「9.4 時間 · 18日」)。空きマスは数えない。
function calendarMonthTotals(cells) {
  let minutes = 0, activeDays = 0;
  for (const c of cells || []) {
    if (!c) continue;
    minutes += c.minutes;
    if (c.count > 0) activeDays += 1;
  }
  return { seconds: minutes * 60, activeDays };
}
// 月を1つ進める/戻す。年をまたぐ繰り上がりをここ1箇所に閉じる。
function calendarShiftMonth(year, month, delta) {
  const d = new Date(year, month + delta, 1);
  return { year: d.getFullYear(), month: d.getMonth() };
}
// 濃さ 0..4。**記録がある日は必ず 1 以上**(長さが保存されていない古いセッションでも、
// 練習した日が「していない日」と同じ見た目になってはいけない)。D-1 から不変。
...
// 色は**既存の紺ランプそのまま**(新しい値を発明しない)。0 = 記録なし = 丸を描かない。
...
// 【D-7 2026/08/23 本人指示】見せ方の切替(窓型 / 折れ線)。本人裁定で**アイコン2つの小さな切替**にした。
// 理由: 画面には既に**子タブ(18px)**と**指標タブ(13px)**の2種類の文字タブがある。
// 3つ目の文字タブを足すと、D-6 で直したばかりの「2つの種類のタブが同じ見た目で
// わかりにくい」が**そのまま再発する**。アイコンなら文字の段と混ざらない。
// 【型】状態を持つ操作なので A型(.ctl-state)…にはしない。**枠を持たない**アイコンで、
// 選択中は色(--c-accent)だけで返す(芯1「枠と違う地を両方与えない」に触れない形)。
function ViewToggle({ value, onChange }) {
  return (
    <div style={{ display: "flex", gap: 2, marginLeft: "auto", flexShrink: 0 }}>
      {MY_DATA_VIEWS.map((v) => {
        const sel = value === v.key;
        return (
          <button
            key={v.key}
            type="button"
          ...
          >
            <v.Icon size={17} strokeWidth={sel ? 2.2 : 1.8} aria-hidden="true" />
          </button>
        );
      })}
    </div>
...
// 大きい数字(--fs-hero)と、その隣の標準偏差を**両方**落とした(本人裁定「両方消す+標準偏差も」)。
// **セッション詳細とリード個別詳細の2画面が同じこの部品を使う**ので、片方だけ消すと
// 部品を2つに割ることになる。本人が「両方消す」を選んだので1つのまま残せた。
// 数字が消えたことで、リードタブは 比較タブ(本人裁定で数字なし)と**全画面で揃った**。
// 受け口 metricsValues は読み手ゼロになったので prop ごと落とし、
...
function SessionEditSheet({
  ...
  //   ・大きさ … **すべて 12px**。行に fontSize を敷いて、値の側(PerformerSelector の bare は
  //     自分で fontSize を持たず親から継承する)まで揃える。以前は 日付13 / 奏者16 / リード12 だった。
  const row = (label, node) => (
    <div className="sans" style={{ display: "flex", alignItems: "center", gap: 8, minHeight: "var(--tap-min)", borderBottom: "1px solid var(--c-line)", fontSize: 12, textAlign: "left" }}>
      <span style={{ fontSize: 12, color: "var(--c-ink-3)", minWidth: "3em", flexShrink: 0 }}>{label}</span>
      {node}
    </div>
  );
  return (
    <BottomSheet ariaLabel="この記録の情報を編集" onClose={onClose}>
      ...
      {row("奏者", (
        <PerformerSelector
          selectId="session-performer-select"
          performers={performers} selectedPerformer={performer || "自分"}
          setSelectedPerformer={onSetPerformer} setPerformers={setPerformers}
          pickerOpen={performerPickerOpen}
          onOpenPicker={() => setPerformerPickerOpen(true)}
...
// 【D-5 2026/08/23 本人指示】メモ欄。本人の実機指摘4つへの答え:
//   ・入力を左詰めに      → 本文は左詰め(以前は右寄せだった)
//   ・縦幅が無駄に大きい  → ラベルを上・本文を下に全幅。右側の空きが無くなる
//   ・1行入ったら次の行へ → <textarea> にして入力量に追従して伸びる
//   ・改行も有効に        → <input> をやめたので改行がそのまま入る
// **セッション詳細とリード詳細で同じ形**(2画面で読み方が変わらない)。
// 【§5】当たり判定は <label> 全体(ラベルを押しても本文にフォーカスが入る)。
// 本文だけだと1行のとき 19px しかないので、<label> に min-height を持たせて満たす。
function MemoField({ value, onChange, onBlur }) {
  const ref = useRef(null);
  // 入力量に追従して伸ばす。scrollHeight を測る前に一度 auto へ戻さないと縮まない。
  ...
  return (
    <label className="sans" style={{ display: "block", minHeight: "var(--tap-min)", cursor: "text" }}>
      ...
      <textarea
        ...
        placeholder="タップして入力"
        className="sans"
        style={{ width: "100%", display: "block", resize: "none", overflow: "hidden", padding: 0, fontSize: "var(--fs-xs)", lineHeight: 1.6, fontFamily: "inherit", color: "var(--c-ink)", background: "none", border: "1px solid transparent", borderRadius: 0 }}
      />
    </label>
  );
}

// 【D-5 2026/08/23 本人指示】選択モード中の「削除」の一手。本人の実機指摘
...
function DeleteActionButton({ count, ariaLabel, onClick }) {
  return (
    <button
      ...
      aria-label={ariaLabel}
      className="sans"
      style={{ ...TAP_BUTTON_RESET, minWidth: "var(--tap-min)", justifyContent: "center", cursor: count > 0 ? "pointer" : "default" }}
    >
      <span
        className="ctl-plain ctl-pill ctl-danger"
        data-armed={count > 0}
        style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "7px 12px", fontSize: 12, fontWeight: 600, lineHeight: 1.2 }}
      >
...
// 【D-5 2026/08/23 本人指示・凍結仕様 design/D5-SPEC.md】マトリクスの1枚ぶん。
...
function PracticeCalendarCard({ sessions, openDayKey, onToggleDay }) {
  ...
  const monthBtn = (delta, label, aria) => (
    <button
      type="button" onClick={() => setYm((p) => calendarShiftMonth(p.year, p.month, delta))}
      aria-label={aria} className="sans"
      style={{
  ...
  return (
    <div onClick={closeDayIfOutside} style={{ paddingTop: 8 }}>
      ...
      <div className="card" style={{ marginTop: "var(--sp-3)" }}>
        ...
        {mySessions.length === 0 && (
          <div className="sans" style={{ fontSize: "var(--fs-xs)", color: "var(--c-ink-3)", paddingBottom: "var(--sp-3)" }}>
...
function AnalysisLabView(props) {
  ...
  // 【D-5】「＋」で足す条件(次元)を選ぶシート。12次元から選ばせる。
  const [addFilterSheetOpen, setAddFilterSheetOpen] = useState(false);
    ...
    <div style={{ maxWidth: 900, margin: "0 auto" }}>
      ...
      <div style={{ paddingTop: 8 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", minHeight: ANALYSIS_ROW_H }}>
          ...
          {/* 【D-5 2026/08/23 本人指示】「絞り込みで追加できる条件が音名だけなので
              それ以外も元の通り追加」。**仕組みは動いていた**(開いた編集の中の「音名 ▾」で
              12次元に変えられる)が、「＋」が必ず音名を足し、変更の入口が編集の中に隠れていたため
              **追加できるのが音名だけに見えていた**。押したら**どの条件を足すか選ばせる**。
              【JSX の罠】この注釈は**開きタグの中に置けない**(属性の位置に波括弧の注釈は書けず、
              esbuild が「Expected ... but found」でビルドを落とす)。必ずタグの外に置くこと。 */}
          <button
            type="button"
            onClick={() => setAddFilterSheetOpen(true)}
            aria-label="集計の条件を追加"
                ...
                return (
                  <div key={i} style={{ display: "flex", gap: 8, alignItems: "flex-start", flexWrap: "wrap" }}>
                    ...
                    <PlainSelect
                      ...
                      value={flt.dimKey}
                      onChange={(v) => setPivotFilters((prev) => prev.map((p, j) => (j === i ? { dimKey: v, values: [], rangeMin: null, rangeMax: null } : p)))}
                      options={PIVOT_DIMENSIONS.map((d) => ({ value: d.key, label: d.label }))}
                    />
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "var(--sp-2)" }}>
          {[
            { key: "row", label: "縦軸", aria: "縦軸を選ぶ",
              ...
              value: pivotRow, onChange: (v) => setPivotRow(v),
              options: PIVOT_DIMENSIONS.map((d) => ({ value: d.key, label: d.label })) },
            { key: "metric", label: "横軸", aria: "横軸(集計する数値)を選ぶ",
              text: PIVOT_MEASURES.find((m) => m.key === pivotMetric)?.label ?? pivotMetric,
              value: pivotMetric, onChange: (v) => setPivotMetric(v),
              options: PIVOT_MEASURES.map((m) => ({ value: m.key, label: m.label })) },
            { key: "col", label: "分析軸", aria: "分析軸(色で分ける単位)を選ぶ",
              text: pivotCol === "none" ? "なし(全体)" : (PIVOT_DIMENSIONS.find((d) => d.key === pivotCol)?.label ?? pivotCol),
              value: pivotCol, onChange: (v) => setPivotCol(v),
              options: [{ value: "none", label: "なし(全体)" }, ...PIVOT_DIMENSIONS.map((d) => ({ value: d.key, label: d.label }))] },
          ].map((z) => (
      ...
      {/* 【D-5】「＋」で足す条件を選ぶシート。**押せない項目を出さない**(F-77)ため、
          既に条件として使っている次元は出さない(同じ次元を2つ足しても片方が効かない)。 */}
      {addFilterSheetOpen && (
        <DataOptionSheet
          ariaLabel="追加する条件を選ぶ"
          items={PIVOT_DIMENSIONS
            .filter((d) => !pivotFilters.some((f) => f.dimKey === d.key))
            .map((d) => ({ key: d.key, label: d.label }))}
          onPick={(k) => {
            setPivotFilters((prev) => [...prev, { dimKey: k, values: [], rangeMin: null, rangeMax: null }]);
            setAddFilterSheetOpen(false);
            setFilterEditorOpen(true);
          }}
          onClose={() => setAddFilterSheetOpen(false)}
        />
      )}
...
function AllSessionsPage({
  ...
  // 【D-5 2026/08/23 本人指示】「すべてのセッション」ページの絞り込みから**期間軸を削除**した。
  // 日付で絞る手段は **PIVOT の条件「録音日」**(範囲入力)に残っている。
  // 消したのは sessionFilterDateFrom / sessionFilterDateTo / dateFilterOpen / dateFilterText と
  // date 入力2つ。奏者・リードの絞り込みは不変。
  ...
  const sessionFilterActive = !!(sessionFilterPerformer || sessionFilterReed);
  ...
  const clearSessionFilters = () => {
    setSessionFilterPerformer(""); setSessionFilterReed("");
  };
...
function SessionDetailView({ session, reeds, sessions, selectedIdeal, promoteSessionToIdeal, updateSessions, performers, setPerformers, tuningHz, onBack }) {
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto" }}>
      ...
      {/* 【D-5 2026/08/23 本人指示】「音階ごとの平均を丸ごと削除」。
          ここにあった表(実音 / 選択中の指標 / 目安との差)は**画面ごと無くなった**。
          正典 #14b も「音ごとの数値を表で見る」を却下側に置いている。
          **音ごとの目安との差を数値で読める場所はこれで無くなる**(グラフの破線は残る)。 */}
      {editOpen && (
        <SessionEditSheet
